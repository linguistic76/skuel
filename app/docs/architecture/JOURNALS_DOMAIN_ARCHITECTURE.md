...
**Persistence (ADR-073):** Text-entry path (`POST /journals/start`) is **zero-persistence** — it runs the workflow and returns the response inline (`HX-Retarget` to `#journal-workspace`), writing **nothing** to Neo4j; the user keeps whatever they copy. File-upload path (still persists pending PR 2): saves compiled output to `je_out/{stem}_out.md` (user downloads and manages themselves, see §7) and writes it to `UserEntry(pipeline=Pipeline.LLM_SUMMARY).processed_content`. Its move to zero-persistence is tracked in `plans/journals-zero-persistence-code-pass.md`.
...
|---|---|---|
| `JournalService` | `core/services/journal/journal_service.py` | Orchestrates both tiers. Seven AI methods: `run_stage1/2/3`, `run_compiled`, `run_standard`, `run_follow_up`, `suggest_activities`. `run_compiled()` is the file-upload batch path: chains all three stages without review gates. `suggest_activities()` runs the LLM DSL bridge over an entry to offer copyable `@context()` lines (the "Suggested activities" panel) — inert; it creates nothing. Does not persist entries — persistence and file output are handled by the calling route. |
...
| `suggestion` | `core/services/journal/suggestion.py` | `SuggestedActivity` + bridge-line → checkbox DSL re-render that **preserves the bridge's tags verbatim** (loose `@when(Friday)`/`@priority(high)` are kept, not dropped — the user refines them). Drops only lines that don't parse, so the panel shows paste-ready `- [ ] … @context(…)` lines. Also owns the panel memo cache: `metadata_with_cached_suggestions` / `read_cached_suggestions` store the result under `entry.metadata["suggested_activities"]`, keyed by a sha256 fingerprint of the source text **and the active-goal grounding** (`grounding_digest` over `JournalService.active_goal_titles`) — so a goal change invalidates the cache even though the entry text is immutable. The route fetches the goal snapshot once and feeds it to both the cache key and `suggest_activities`, so a cached panel is always keyed by the grounding that produced it (no fetch-twice skew). The route also gates on `JournalService.suggestions_available` so a bridge-unavailable empty result (FULL tier, no OpenAI key) is never cached. |
| Routes | `adapters/inbound/journals_routes.py` | 12 routes. FOUNDER enforcement (`journal_tier.is_founder()`) lives at the route layer. Text-entry path: `POST /journals/start` runs AI and returns the response **inline** (`HX-Retarget` `#journal-workspace`) — zero-persistence (ADR-073): no `UserEntry`, no redirect. The dedicated chat page `GET /journals/{entry_uid}` is reached by the file-upload path (PR 2 will make upload zero-persistence too), with a lazy-loaded "Suggested activities" right column (`POST /journals/suggest-activities`, ownership-checked, FULL tier; memoised under `entry.metadata` so a revisit skips the LLM bridge unless the text changed). File-upload path saves compiled output to `je_out/{user_uid}/{stem}_{entry_uid}_out.md` and also redirects to the chat page. `GET /journals/je-out/{filename}` serves `je_out/` files with a path-containment guard. |
