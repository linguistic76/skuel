...
def create_teaching_api_routes(
    ...
    Args:
        ...
        revised_exercise_service: RevisedExerciseService for creating RevisedExercises
    """
    ...
    async def request_revision(request: Request, uid: str, current_user: Any) -> Any:
        ...
        Two-phase: (1) create ExerciseReport via teacher_review_service,
        (2) create RevisedExercise with structured instructions and feedback points.
        """
        ...
        # Phase 1: Create ExerciseReport (existing behavior)
        ...
        # Phase 2: Create RevisedExercise if service is available
        if revised_exercise_service and form_data.exercise_uid:
            revision_data = revision_result.value
            report_entity_uid = revision_data.get("report_uid", "")
            student_uid = revision_data.get("student_uid", "")
            ...
            entity = RevisedExercise(
                ...
                title="",  # Auto-generated by service
            ...
            create_result = await revised_exercise_service.create(entity)
            if create_result.is_error:
                # ExerciseReport was still created — log but don't fail the whole request
                logger.warning(
                    f"ExerciseReport created but RevisedExercise creation failed: "
                    f"{create_result.expect_error().message}"
                )
                return Div(
                    P(
                        "Revision requested (report created), but structured revision "
                        "instructions could not be saved.",
                        cls="text-sm text-amber-600 font-medium",
                    ),
                    cls="p-3 bg-amber-50 rounded border border-amber-200",
                )

            re_uid = create_result.value.uid
            return Div(
