...
**Activity Domains are equal entry points.** A user's Tasks, Goals, Habits, Events, Choices, and Principles are knowledge-in-action. They receive the same feedback infrastructure as curriculum work — via `AiFeedback` entities generated by AI or written by admins.

**See:** [FEEDBACK_ARCHITECTURE.md](/docs/architecture/FEEDBACK_ARCHITECTURE.md) for the complete feedback taxonomy.

## The Four EntityTypes in the Submission/Feedback Loop
...
| `AI_FEEDBACK` | System **or** Admin | Activity-level feedback (not tied to artifact) | `AUTOMATIC`, `LLM`, or `HUMAN` |
| `FEEDBACK_REPORT` | Teacher **or AI** | Assessment with `subject_uid` pointing to student's submission | `HUMAN` (teacher) or `LLM` (AI) |

**Note:** `AI_FEEDBACK` does **not** inherit from `Submission`. It has no file fields. It inherits `UserOwnedEntity` directly and responds to a user's aggregate activity patterns over a time window.
