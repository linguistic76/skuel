"""
Unit tests for TaskKnowledgeAnalyzer — knowledge analytics and learning pattern detection.
...
def make_mastery_progression(knowledge_uid: str = "ku.python") -> MasteryProgression:
    ...
    return MasteryProgression(
        ...
        validation_activities_passed=3,
...
@pytest.fixture
def engine_no_svc() -> TaskKnowledgeAnalyzer:
    """TaskKnowledgeAnalyzer with no relationship_service — pattern detection returns empty."""
    return TaskKnowledgeAnalyzer(relationship_service=None)
...
@pytest.fixture
def engine_with_svc(mock_relationship_service: Mock) -> TaskKnowledgeAnalyzer:
    return TaskKnowledgeAnalyzer(relationship_service=mock_relationship_service)
...
class TestAnalyzeLearningPatterns:
    ...
    @pytest.mark.asyncio
    async def test_returns_result_ok_type(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
...
class TestCalculateKnowledgeAwarePriority:
    ...
    @pytest.mark.asyncio
    async def test_scoring_rationale_is_list(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
...
class TestGenerateTaskInsights:
    ...
    @pytest.mark.asyncio
    async def test_returned_insights_is_list(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
...
class TestIsProgressiveSequence:
    ...
    def test_fewer_than_3_elements_returns_false(
        ...
        assert engine_no_svc._generic_analyzer._is_progressive_sequence([1.0, 2.0]) is False
...
class TestCalculateGrowthIndicator:
    ...
    def test_single_value_returns_zero(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
        """Only one element → cannot compute growth → 0.0."""
        assert engine_no_svc._generic_analyzer._calculate_growth_indicator([5.0]) == 0.0

    def test_empty_list_returns_zero(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
        """Empty list → 0.0."""
        assert engine_no_svc._generic_analyzer._calculate_growth_indicator([]) == 0.0

    def test_growing_values_returns_positive(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
        """Second half mean > first half mean → positive growth."""
        result = engine_no_svc._generic_analyzer._calculate_growth_indicator([1.0, 1.0, 2.0, 2.0])
        assert result > 0.0

    def test_declining_values_returns_negative(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
        """Second half mean < first half mean → negative growth."""
        result = engine_no_svc._generic_analyzer._calculate_growth_indicator([2.0, 2.0, 1.0, 1.0])
        assert result < 0.0

    def test_flat_values_returns_zero(self, engine_no_svc: TaskKnowledgeAnalyzer) -> None:
