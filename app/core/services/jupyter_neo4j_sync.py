...
class JupyterNeo4jSync:
    ...
    async def save_from_jupyter(
        ...
        try:
            ...
            update_result = await self.backend.update_entity_from_jupyter(
                {
                    "uid": uid,
            ...
            await self.backend.replace_relationships(
                uid,
            ...
            result = await self.backend.track_change(uid, "jupyter_edit", new_hash)
            if result.is_error:
