...
| `adapters/inbound/journals_routes.py` | `POST /journals/start` (text entry → runs AI → returns response **inline**, `HX-Retarget` `#journal-workspace`; persists an owner-private `:ConversationSession` + turns for revisit/continue, **no UserEntry** — ADR-078 understanding-agnostic store); `POST /journals/follow-up` branches on `session_id` (session-backed reads/appends turns, 404-not-403; file/audio doors stay stateless); `POST /journals/upload` + `POST /journals/folder-process` process to the user's own `je_out/` folder via one shared stateless batch engine (no UserEntry); `GET /journals/{entry_uid}` is **periodic-notes-only**; `GET /journals/je-out/{filename}` downloads a flat `je_out/` file |
