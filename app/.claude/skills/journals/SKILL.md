...
| Follow-up (conversation continuation) | `run_follow_up(original_entry, ai_response, user_reply, user_uid, mode, summon_canon=False)` | `follow_up_system_prompt()` — mode base + continuation directive. Canon **quote-on-demand** surface (ADR-076): `summon_canon` retrieves on `user_reply`, injects `CanonContext.to_discussion_block()` (name + quote the shelf verbatim, faithfulness contract), appends a Sources footer with location + Resource link | 4000 |
