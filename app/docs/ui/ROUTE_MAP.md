...
One navigation, one rule (`ui/layouts/nav_config.py`): the chrome carries one door per SECTION, lit by a section key; a section's pages are its sidebar's rows. Section doors (`ICON_NAV_ITEMS` — the desktop centre links at sm+ AND the phone bottom-nav tabs below, the same four for every authenticated role, in order): **Tasks+** (`/today`, lit on every activity-sidebar page — Today, the calendar views, the six domains, the periodic notes, the GradeBook) → **Library** (`/explore/library`, lit on `explore` and `library`) → **PathSteps** (`/path-steps`) → **Submissions** (`/submissions`). An anonymous visitor gets Library + PathSteps. Role doors (`MAIN_NAV_ITEMS`): **Teaching** (`/teaching/students`) and **Admin** (`/admin`) — centre links at lg+, rows on `/settings` below. Right section (icon buttons): **Askesis** (`/askesis`) → **Shared** (`/profile/shared`) → notification bell → avatar (`/settings`, the account page — preferences, devices and, on phones, the sign-out row) → **Sign out** (`/logout`, sm+ only). The SKUEL brand link goes to `/explore` for authenticated users; `/` is a 303 to `/today`.

### `/groups` — Student-Facing Group-Shares Hub

Tabbed layout with one tab per group the student is a member of (capped at `MAX_STUDENT_GROUPS = 4`, enforced server-side in `GroupService.add_member()`). Each tab shows a single "Recent Shares" block HTMX-loaded from `GET /api/groups/{group_uid}/shared/preview` — peer-authored `UserEntry`s shared with the group (`(entry)-[:SHARED_WITH_GROUP]->(group)`; a turn-in sent to the teacher is `SUBMITTED_TO_GROUP` and never appears here, ADR-088 §2). The list is the Shared page's own reader narrowed to the group (`UnifiedSharingService.get_shared_with_me(via=group_uid)`), gated by the one audience fragment and excluding the viewer's own entries, so whatever is listed opens at `/gradebook/{entry_uid}` (the recipient card). Zero-group students see an `EmptyState`. UI in `ui/groups/hub.py` + `ui/groups/shared_preview.py`; routes in `adapters/inbound/groups_hub_routes.py`. Distinct from `/teaching/groups` (teacher-facing group management).
...
Two sides (Submit & Share arc R7). **Shared with you**: every `UserEntry` (and `FormSubmission`) the share links name the viewer for — a person share (`SHARES_WITH`) or a share with an active group they belong to or own (`SHARED_WITH_GROUP`) — as the R6 card: title, description, from (the owner), date, the "Shared with you" badge, the derived review badges ("Revised after feedback" / "Reviewed · Teacher|AI" — `ui/gradebook/review_badges.py`, derived by the list read through `build_review_standing_subquery`, never stored; ADR-088 §1, R2), via chips ("directly" / the group names) and an Open link to `/gradebook/{uid}` (the recipient card). Never feedback: reports, revision requests and activity reports live in the GradeBook (R3). A FilterBar (Type · Shared by · Via, options derived from the live list) narrows the cards server-side through the `/profile/shared/list-fragment` HTMX fragment. **Your wall**: the viewer's own shared entries, one row per entry with its review badges and an audience chip per person and group, each with × Stop sharing (`POST /api/user-entries/{uid}/unshare`, which swaps the row); visible to its owner only. Sharing itself happens on the entry's page: the Share button on `/gradebook/{uid}` (and the exchange thread's per-version "Share →" link) opens a panel HTMX-loaded from `/gradebook/{uid}/share-panel` — candidate groups and co-members as checkboxes — posting to `POST /api/user-entries/{uid}/share`; a new person share rings the recipient (`shared_with_you`). Reached from the inbox icon in the top navbar (next to the bell). View in `ui/profile/shared_view.py`; route in `adapters/inbound/user_profile_ui.py`.
...
THE received-feedback page (3→1 collapse, feedback-loop UX arc 2 C1+C2) — one exchange line per exercise, newest activity first: exercise title, derived status (**Waiting** / **Feedback received** / **Revision requested** — `ExchangeStatus.derive`), latest-activity date, source of the latest feedback (Teacher/AI), and lineage counts; each line opens its `/exchange` thread. Lines are keyed by the entries' turn-in snapshot (`UserEntry.turn_in_exercise_uid` / `turn_in_exercise_title`, stamped by the writer), so an exchange outlives its exercise: a deleted exercise's line stays, titled from the snapshot and badged **Exercise removed** (Submit & Share arc R12). Status chips + a Source select filter server-side through the `GET /gradebook/lines` HTMX fragment (one summary read: `get_student_exchange_summaries`, `_UserEntryReportQueryMixin`). Two conditional groups render below only when non-empty: **Activity reports** (flat list; request form at `/submit-activity-report`) and **Other feedback** ("Feedback on work that isn't tied to an exercise." — reports on entries that are not turn-ins, R13). Route in `adapters/inbound/user_entry_ui.py` (`gradebook_page`); view in `ui/gradebook/summary.py`. The page renders under the **Tasks+ sidebar** (`render_activity_sidebar_page`, GradeBook row lit) — it has no sidebar of its own; the header's "Request activity report" action opens `/submit-activity-report`, which renders under the same sidebar. An exchange whose latest entry is a post-feedback revision not yet shared carries the "Share your revised work" nudge beneath its line (ADR-088 §1, R2): `/gradebook/{entry_uid}?share=1&preselect=reviewers` opens the entry's page with the Share panel open and the groups the entry was submitted to for feedback checked (`latest_entry_revised_after_feedback` / `latest_entry_shared` on `StudentExchangeSummary`, derived by the summaries read).
...
The `/gradebook/{uid}` route is viewer-aware (Submit & Share arc R6, ADR-088 §3): one audience read (`read_visibility` `OWNER_OR_AUDIENCE`) admits the owner and whoever the share links name — a direct `SHARES_WITH`, or membership / ownership of an active group the entry is `SHARED_WITH_GROUP` to — and anyone else gets the rendered not-found at a real 404. A recipient sees the card in `ui/gradebook/recipient_card.py` (title, description, from, date, a "Shared with you" badge beside the derived review badges — "Revised after feedback" / "Reviewed · Teacher|AI", read by `ReportRelationshipService.get_entry_review_standing` after the audience read admitted the viewer — and "Open file (.md)" → `/gradebook/{uid}/download`, which serves the title, description and body as `text/markdown` behind the same read) and never the status, the processed body, feedback or the exchange. The owner's page renders submission detail for the `UserEntry` — including a fulfills-exercise badge (read from the turn-in snapshot: the live exercise's title while it exists, the snapshotted title plus an **Exercise removed** badge once it is deleted) with a "View exchange thread →" link, a "Request AI feedback" button (submission owner, FULL tier — posts to `POST /api/exercises/report`), and a "Map of Content" card section when the entry has outgoing `ORGANIZES` edges (emergent MOC, drawn by vault `moc: true` ingestion) — children link to their per-type detail pages via `ui/patterns/entity_links.py`.

### `/exchange?exercise={uid}[&student={uid}]`

Read-only exchange thread (feedback-loop UX arc C5) — one (student, root exercise) exchange rendered chronologically: submissions (all revisions, including entries against a `RevisedExercise`), feedback reports, and revision requests (labelled with `EntityType.REVISED_EXERCISE`'s display name, "Revision request"), each linking to its existing detail/action surface per viewer (student → `/gradebook/{uid}` / `/entry-reports/detail`; teacher → `/teaching/review/{uid}`). The chain is keyed by the entries' turn-in snapshot, so the thread opens after the exercise is deleted — the header shows the snapshotted title with an **Exercise removed** badge and no exercise link (Submit & Share arc R12); not-found means the student has no turn-in against that uid. The viewer reads their own exchange; `student=` requires both report-download gates — the live TEACHER role plus a shared ACTIVE owned group — and the teacher-mode chain shows only entries `SUBMITTED_TO_GROUP` an active group the viewer owns (a multi-class student's other classroom stays invisible). Every denial serves the same rendered not-found page with a real HTTP 404 (404-not-403). Entry points: the Shared-With-Me card's exercise subject link, `/gradebook/{uid}`, the `/teaching/review/{uid}` fragment, and the learning-loop PS submissions rows. Route: `adapters/inbound/exchange_ui.py`; renderer: `ui/learning_loop/exchange_thread.py`.
...
- `/submissions/submit` — the Submit page: an optional Title, "Ask for feedback?" (Teacher — with or without an exercise — / AI — only with one — / No), "Share with" (collapsed: the student's groups and co-members, Portfolio coming-soon), the file. `?exercise_uid=` names the exercise the turn-in answers (+ `from_ps`). The one Submit route.
...
- `GET  /journals/daily` — today's daily note (find-or-create + redirect); the Tasks+ sidebar's Journal row, resolved at click time
- `GET  /journals/daily/{date}` · `GET  /journals/weekly/{year}/{week}` · `GET  /journals/monthly/{year}/{month}` · `GET  /journals/quarterly/{year}/{quarter}` · `GET  /journals/yearly/{year}` — find-or-create the period's note (`UserEntryService.ensure_periodic_note` owns the UID scheme), then redirect to it. Declared **above** the `{entry_uid}` catch-all (FastHTML resolves in declaration order); an out-of-range quarter/year degrades to the current one rather than 500ing.
- `GET  /journals/{entry_uid}` — **periodic-notes-only** (`entry_kind` ∈ `PERIODIC_NOTE_KINDS`: daily, weekly, monthly, quarterly, yearly) → `PeriodicNotePage` inside the **Tasks+ sidebar page** (`render_activity_sidebar_page`, Journal row lit, `max-w-none`): the note editor with the **period navigator** to its right (under it below `lg`) — a ← Calendar link, a mini month grid whose day cells open daily notes and whose leading ISO-week rail opens weekly notes (the same two doors the full month grid carries), and below it a **period rail** — one row per period kind (daily → yearly, the note's own row marked current), each opening that period's note and stepping to its neighbours with ‹ › arrows. The rail is the in-note door to every period; the top-level door is the sidebar's Journal row (the navbar carries no note door). No planning panel — the period's entities are read on the calendar views and Today. Any non-periodic uid → 404 (discussions are served by `/journals/discussion/{session_id}`, not this route).
