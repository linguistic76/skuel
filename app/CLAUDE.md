...
- **Regular user navbar** icon links (left section, in order): **Activity** (`/profile`, icon `activity`) + **Explore** (`/explore`) + **GradeBook** (`/gradebook`) + **Library** (`/library`) + avatar **U** (click → Profile + Search + 6 Activity Domain links + Sign out dropdown).
