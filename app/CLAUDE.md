...
| SKUEL023 | `self.backend` in `core/` must name a `core/ports` protocol — not a concrete adapter class (direction), and not `Any`/unannotated (strength). Strength fires on classes that **assign** `self.backend` *and* on declaration-only class-body `backend: Any` (the mixin shape — dead declarations included, where the fix is deletion); Scope D (`backend` inherited from a `Base*[Any, ...]` parameterisation) is not yet covered | ERROR |
