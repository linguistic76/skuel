...
| `@ku()` | primary knowledge link — the Ku's stored uid | `@ku(ku.sel.mindfulness)` |
| `@link()` | goal:, principle:, ku:, ps:, lp: + the entity's stored uid, copied from its page | `@link(goal:goal.health.strong-body, principle:principle.sel.awareness-first)` |
...
```markdown
- [ ] Draft Teens.yoga lesson on focus @context(task,learning) @when(2026-07-14T09:00) @priority(1) @duration(90m) @energy(focus,creative) @ku(ku.teens-yoga.focus-lesson) @link(goal:goal.teens-yoga.twenty-members)
```
