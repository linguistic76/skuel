...
| Follow-up | `run_follow_up(original_entry, ai_response, user_reply, user_uid, mode=None, summon_canon=False)` | `follow_up_system_prompt()` — mode base + continuation directive (no re-analysis). **Canon quote-on-demand surface (ADR-076):** `summon_canon` retrieves on `user_reply`, injects `to_discussion_block()` (may name + quote the shelf verbatim), appends a Sources footer | 4000 |
