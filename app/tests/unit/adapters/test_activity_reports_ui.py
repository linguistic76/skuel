...
@pytest.fixture
def prompt_pages(monkeypatch: pytest.MonkeyPatch):
    """Render the sidebar shell as a dict so the prompt's content is inspectable."""
...
class TestForPeriodGenerate:
    ...
    async def test_generates_the_period_and_lands_on_the_report(
        ...
        registry, _, generator = registry_orchestrator_generator
        generator.generate = AsyncMock(
            return_value=Result.ok(_report(uid="ar_new", time_period="2026-01"))
        )
        handler = registry.get("/activity-reports/for", "POST")

        response = await handler(_make_request(form_data={"time_period": "2026-01"}))

        assert response.status_code == 302
        assert response.headers["location"] == "/activity-reports/detail?uid=ar_new"
        generator.generate.assert_awaited_once_with(user_uid="user_reports", time_period="2026-01")

    @pytest.mark.asyncio
    async def test_cooldown_refusal_re_renders_the_prompt_with_the_reason(
        self, registry_orchestrator_generator, prompt_pages
    ):
        registry, _, generator = registry_orchestrator_generator
        generator.generate = AsyncMock(
    ...
    async def test_unknown_token_is_400_before_any_generation(
        ...
        response = await handler(_make_request(form_data={"time_period": "someday"}))

        assert response.status_code == 400
        generator.generate.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_missing_csrf_is_refused(self, registry_orchestrator_generator):
        registry, _, generator = registry_orchestrator_generator
        handler = registry.get("/activity-reports/for", "POST")
        request = SimpleNamespace(
            session={"user_uid": "user_reports"},
            url=SimpleNamespace(path="/activity-reports/for"),
            method="POST",
            cookies={},
            headers={},
            query_params={},
            form=AsyncMock(return_value={"time_period": "2026-01"}),
        )
