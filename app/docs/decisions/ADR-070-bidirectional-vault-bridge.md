...
| Checkbox status undone (`[ ]`) | **SKUEL, for its own writes** | SKUEL→vault | Amended 2026-08-24: re-opening in SKUEL un-checks the line and strips the `✅` date it wrote. Gated on the `✅` token, so a dateless `[x]` the USER ticked is left alone. The vault→SKUEL direction was never built (see the status annotation) |
