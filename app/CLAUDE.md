...
- SKUEL024: No `cls=` + `**kwargs` collision in FT helpers [ERROR] — a helper that hardcodes `cls="..."` AND splats `**kwargs` into the same call without an explicit `cls` param raises `TypeError: got multiple values for keyword argument 'cls'` the moment a caller passes `cls=` (this 500'd `/insights`). AST rule (non-test files); `kwargs.pop("cls")` defusal is exempt. Fix: add `cls: str = ""` and merge — `cls=f"...base... {cls}".strip()`. Contract guarded by `tests/unit/ui/test_cls_merge_contract.py`.
