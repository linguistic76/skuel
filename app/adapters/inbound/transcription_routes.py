...
Clean routes for the simplified TranscriptionService.
...
logger = get_logger("skuel.routes.transcription")


def create_transcription_routes(
    _app: Any,  # Unused - kept for route factory interface pattern
    rt: Any,
    ...
    Args:
        app: FastHTML app instance (unused - kept for interface consistency)
        rt: Route decorator
    ...
    async def create_transcription(request) -> Result[Any]:
        ...
        result = await service.create(create_request, user_uid)
    ...
    async def process_transcription(request, uid: str) -> Result[Any]:
        ...
        result = await service.process(uid, options)
    ...
    async def retry_transcription(_request, uid: str) -> Result[Any]:
        ...
        result = await service.retry(uid)
    ...
    async def search_transcriptions(request) -> Result[Any]:
        ...
        result = await service.search(query, user_uid=user_uid, limit=limit)
    ...
    async def get_by_status(request, status: str) -> Result[Any]:
        ...
        result = await service.get_by_status(status_enum, user_uid=user_uid, limit=limit)
    ...
    logger.info(f"Transcription routes registered: {len(routes)} endpoints")
