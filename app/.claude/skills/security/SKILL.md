...
| **Vault sync privacy wall** | Two layers at the shared ingestion chokepoint (`collect_files` + `ingest_file`, so reconciler, `/api/ingest/directory`, `/api/ingest/file` all inherit): (1) **staging floor** — `je_*` folders excluded unconditionally, in any config; (2) **fail-closed allowlist** (`SyncAllowlist` / `SKUEL_VAULT_SYNC_ALLOWED_DIRS`) — only allowed folders under the personal vault root ingest (`periodic_notes/` by default when unset), symlink-safe, configured dirs must be strictly under the root | Active (default-on) |
