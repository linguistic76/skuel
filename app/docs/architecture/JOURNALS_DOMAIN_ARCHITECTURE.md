...
| `suggestion` | `core/services/journal/suggestion.py` | `SuggestedActivity` + bridge-line → canonical checkbox DSL re-render. Drops malformed lines so the panel only shows paste-ready `- [ ] … @context(…)` lines. |
