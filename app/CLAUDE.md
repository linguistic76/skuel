...
- `/profile` is lean (Focus + Steady + Settings). Activity domains at `/activities/{domain}` with sidebar. `/learn` is the student workspace hub (Submit/Exercises/My Submissions/Reports sidebar on sub-pages). Old `/submissions/*` UI paths redirect 301 to `/learn/*`.
