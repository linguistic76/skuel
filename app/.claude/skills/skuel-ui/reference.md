...
- **Activity Domains** — `render_activity_sidebar_page()` from `ui/activities/nav.py` — `ACTIVITY_SIDEBAR_ITEMS` (Today, Weekly, Monthly, Tasks, Goals, Habits, Principles, Choices, Journal) on `/tasks`, `/goals`, `/habits`, `/choices`, `/principles` and the events pages (which highlight Monthly — the month view is the events lens); the calendar month/week pages and `/today` pass `items=CALENDAR_SIDEBAR_ITEMS` — the slimmer variant (Today / Weekly / Monthly / Journal / Reports → `/activity-reports/latest`), no domain rows and no `/api/sidebar/badges` request (`content_max_width="max-w-none"` on the calendar). Each calendar view shows its declared membership (`VIEW_SPECS`: month = events; week = events, habits, goal milestones and high-priority tasks, with the kind legend as filter).
