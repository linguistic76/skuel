...
- **Richer UserContext injection** — ✅ shipped (ADR-081 D2): grounding now rides `UserContextBuilder.build()` through the curated projection (relevance-annotated goals/tasks/habits, learning journey). Further enrichment = extending `JOURNAL_GROUNDING_FIELDS` deliberately, not new plumbing.
