...
remaining **327 findings / 210 distinct targets** now live in
[`dead-doc-links-sweep-queue.md`](dead-doc-links-sweep-queue.md), with the per-directory
split, the tail shape that decides the fix (only 33 of 210 have a unique same-basename
...
| ~~Dead-doc-links B1–B5~~ — **ARC COMPLETE 2026-09-02** (#1217, #1218, #1219, B4, B5) | — | `uv run python scripts/health/dead_doc_links.py` — **327** today, all of it the live-docs sweep queue row below; `decisions/` and the history dirs are clear. Re-measure, never trust the snapshot. See the section |
