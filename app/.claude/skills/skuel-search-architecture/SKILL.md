...
**A surface's result scope is NOT `_SEARCHABLE_DOMAINS` (August 2026):** the `/search` page searches the **6 Activity Domains + Ku** — `SEARCH_PAGE_ENTITY_TYPES` / `scope_to_search_page` in `adapters/inbound/search_routes.py`. PathStep, LearningPath, Exercise, RevisedExercise and UserEntry are off that page, so the "My Entries" dropdown option no longer reaches entries. Declare a surface's scope at its own entry point and narrow the REQUEST; never edit the router's shared sweep default — `/explore` and `/explore/library` ride the same `faceted_search`. Body-chunk augmentation follows the request's entity types too, so an excluded domain cannot re-enter through the Digital layer. **See:** `docs/architecture/SEARCH_ARCHITECTURE.md`.
