...
The image ships **no default `server.jvm.additional` lines** (verified on `2026.05.0`: the live JVM
carries only `-Xms`/`-Xmx` from the memory settings, no GC flags — modern JDK defaults to G1GC
implicitly). So `NEO4J_server_jvm_additional` is **purely additive** here — it does not clobber GC or
heap flags. If a future image ever ships default `jvm.additional` lines, this var would need to
re-list them (the setting is multi-valued).
