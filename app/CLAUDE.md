...
- **Two doors reach `UserEntryService.create_entry()`, with no shared middle layer** (ADR-054). The exercise turn-in is one: `/submit` is a legacy 302 onto the canonical `/submissions/exercise`, whose form HTMX-posts to `POST /api/user-entries/upload`, and that handler in `adapters/inbound/user_entry_api.py` builds the request and calls `create_entry()` itself. `ingest_user_entry` (`core/services/ingestion/user_entry_ingestion.py`) is the OTHER door — the vault/YAML one, with exactly one caller, `UnifiedIngestionService`, which reaches it from `ingest_file()` and therefore **is** on the directory-ingest path. `create_entry()` is the single convergence point; the turn-in is the door that never touches directory ingest. The vault is the source of truth for user data; `/settings/vault` (Obsidian bidirectional sync) is the primary personal-data path.
