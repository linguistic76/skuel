...
- **Right (icon buttons):** Search (`/search`) and Calendar (`/cal`, `page_key="calendar"`) — both desktop only, mobile folds them into the bottom nav (a sixth always-visible top-bar icon would break the 44px tap-target minimum at 320px) → Askesis flame (`/askesis`) → Shared-inbox (`/profile/shared`) → notification bell (HTMX lazy-loaded badge from `/api/navbar/notification-badge`) → Profile avatar (`/profile`) → Sign out (`/logout`, desktop only — the phone reaches it from the `sm:hidden` row on `/profile`, so exactly one door exists at every width)
