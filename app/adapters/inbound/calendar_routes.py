"""
Calendar Routes - Unified Temporal View
========================================

FastHTML routes for the calendar system that displays tasks, events, and habits
in day/week/month views with interactive features.

Architecture:
    - UI Components: /components/calendar_components.py
    - Alpine.js: Modal and drag-drop via /static/js/skuel.js
...
def create_calendar_routes(app, rt, services):
    """
    Create calendar routes for the application.

    Args:
        app: The FastHTML app instance
        rt: The router instance
        services: Services container with calendar service
    """
    ...
    async def reschedule_item(request: Request) -> Any:
        ...
        if result.is_ok:
            # Return empty response with HX-Refresh to reload page
            return Response(
                content="",
    ...
    async def calendar_habit_record(request: Request, habit_uid: str, status: str) -> Any:
        ...
        try:
            ...
            if services.habits:
                result = await services.habits.record_occurrence(
                    habit_uid=habit_uid,
                    on_date=today,
                    status=status.upper(),
                    notes=notes or None,
    ...
    logger.info("✅ Calendar routes created (HTMX-enhanced)")
