...
def _palette_icon() -> NotStr:
    """Create a palette SVG icon for theme switching."""
    return NotStr(
        '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" '
        'stroke-width="1.5" stroke="currentColor" class="size-5" aria-hidden="true">'
        '<path stroke-linecap="round" stroke-linejoin="round" '
        'd="M4.098 19.902a3.75 3.75 0 0 0 5.304 0l6.401-6.402M6.75 21A3.75 3.75 0 0 1 3 17.25V4.125C3 '
        '3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 0 0 3.75-3.75V8.197'
        'M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072M10.5 '
        '8.197l2.88-2.88c.438-.439 1.15-.439 1.59 0l3.712 3.713c.44.44.44 1.152 0 1.59l-2.879 2.88M6.75 17.25h.008v.008H6.75v-.008Z"/>'
        "</svg>"
    )


def _theme_switcher() -> Div:
    """Create a theme switcher dropdown using the themeSwitcher Alpine component."""
    return Div(
        Button(
            Span("Switch theme", cls="sr-only"),
            _palette_icon(),
            type="button",
            cls="btn btn-ghost btn-circle btn-sm text-base-content/70 hover:text-base-content",
            **{"@click": "toggle()"},
        ),
        # Dropdown panel
        Div(
            Div(
                Div(
                    Span("Theme", cls="text-xs font-semibold uppercase tracking-wide text-base-content/50"),
                    Span(**{"x-text": "current"}, cls="text-xs font-medium text-primary"),
                    cls="flex items-center justify-between px-3 py-2 border-b border-base-200",
                ),
                Div(
                    NotStr(
                        '<template x-for="theme in themes" :key="theme">'
                        '<button @click="setTheme(theme)" '
                        ':class="theme === current ? \'bg-primary text-primary-content\' : \'hover:bg-base-200\'" '
                        'class="w-full text-left px-3 py-1.5 text-sm rounded transition-colors" '
                        'x-text="theme" type="button"></button>'
                        '</template>'
                    ),
                    cls="p-2 grid grid-cols-2 gap-1 max-h-64 overflow-y-auto",
                ),
                cls="bg-base-100 border border-base-300 rounded-lg shadow-xl w-56",
            ),
            **{
                "x-show": "open",
                "x-cloak": "",
            ...
            cls="absolute right-0 top-full mt-1 z-50",
        ),
        cls="relative",
