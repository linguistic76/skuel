...
| History directories | `HISTORY_DIRS` (`docs/migrations`, `docs/roadmap/done`, `docs/investigations`, `docs/Reviews`, `docs/design-handoff`); skipped in `get_md_files()` | Dated records of a past state, where a dead link is the record being **faithful** — the file really was there when the record was written — or, for `docs/design-handoff/`, a design record whose own header says "do not follow as instructions". Directory membership IS the classification, so this one is a directory carve-out (Mike's ruling, 2026-09-01; `design-handoff/` added 2026-09-20). ⚠️ `docs/roadmap/done/` and never `docs/roadmap/`: the live half is "what might still happen" and its dead links are ordinary rot |
