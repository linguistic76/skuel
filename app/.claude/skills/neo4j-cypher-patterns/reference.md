...
| Relationship | From | To | Purpose |
|--------------|------|-----|---------|
| `EXTRACTED_FROM` | created Entity (Task, Habit, ...) | UserEntry (source) | Extraction provenance; carries `extracted_at`, `source_line_hash` (sha256 of the DSL line normalized for whitespace, checkbox state and the 🆔 token — `normalize_vault_line_hash`, the re-run dedup key; the ✅ date stays in it as a discriminator), `vault_id` (the 🆔 — a line whose 🆔 already has an edge to the entry is recognised by it whatever its hash, Guard 2b), `source_line` (the line verbatim as SKUEL last saw it — the base of ADR-070 Decision 3's three-way merge; written on create, seeded where absent, carried unchanged by a re-point and a revival, advanced on an ok reconciliation write or a verdict with nothing to write — `advance_extracted_from_links` — and by SKUEL's own outbound writes, each applied to the base as to the file — `_OwnWrite`). The edge lives as long as its line: gone by both keys (🆔 absent, digest on no 🆔-less line) ⇒ retired on re-ingest; the Task is stamped `retired_vault_id` / `retired_source_line` / `vault_line_retired_at` by the same statement (the one-sync grace record; a re-appearing 🆔 revives the edge; the end-of-sync sweep judges every stamp older than the sync: terminal or still tracked from another note ⇒ stamp cleared, open and untracked ⇒ CANCELLED through the Tasks facade, stamp cleared on ok — and holds everything over an incomplete inbound pass) |
