...
5. **`status:` must be legal for the entity type, not merely a real status.** Membership (criterion 4) and legality are different questions: `completed` is a real `EntityStatus` and is not in `EntityType.PRINCIPLE.valid_statuses()`, and `draft` is not in `EntityType.EVENT`'s. `validate_entity_data` refuses both, naming the legal set — the same per-type check the status-guarded write applies (`validate_status_target`, ADR-087), called here because the bulk vault upsert bypasses that write. Same severity as criterion 4 (content fault, ignored with a reason), same `user_entry` exemption and the same reason for it, and the same reading of absence — the `none` marker and an empty `status:` line prepare to null, which is not a value to judge (the entity reader supplies the type default for a status that was never stored); `group` and the other `NonKuDomain` types have no `EntityStatus` lifecycle and are skipped. The legal set per type is tabulated in § Entity Status of [the YAML authoring guide](../guides/YAML_AUTHORING_GUIDE.md).
...
`from_string` aliases (`status: pending` stored as `draft` — the graph always stores
canonical), and the authored `none` absence marker (property dropped at the upsert,
which also cleans a previously-persisted `"none"` string on re-sync). Values outside
all three tolerances fall through untouched to `validate_entity_data`'s vocabulary
gate, which rejects the file (§ Minimum frontmatter, criterion 4) — so the graph
never meets non-canonical casing, an alias spelling, OR a non-member value in a
registered field, and exact-match property filters (the faceted `sel_category`
facet) stay sound.
...
A sync whose only findings are ignored files **is clean** ("Sync complete" + the ignored list). The reason text keeps three flavors distinct: a file with no `type:` at all (likely a deliberate non-entity note); a file that *declared* a type but has a malformed field — rendered as `path — declared 'type: X' but not ingested: reason`, easy to spot for fixing; and a file whose frontmatter fence does not parse at all, which can declare nothing and reports as `Invalid YAML frontmatter at line N, column M` (the line is the author's file line, not YAML's offset inside the fence). The default-deny gate is unchanged — nothing is inferred or ingested; only severity and reporting changed.
