...
| [Askesis Intent Classification — Activation Arc](roadmap/askesis-intent-classification-activation.md) | Turning on the intent-conditioned Askesis layer, which has never executed: the 0.65 gate is a MEAN over 8 exemplars and is unreachable. PR-1 + baseline RATIFIED 2026-08-31 (`./dev eval-intent-classification`, 45 labelled queries) — the classifier RANKS 30/31 correctly, so the gate is the whole defect and PR-2 moves it. Activates the two answer-shaping branches (graph context, suggested actions); the chunk-type filter stays off as a gated PR-3. `AGGREGATION` STAYS (ruled 2026-08-31, position b) — its collision with `EXPLORATORY` is disambiguated instead, and that is a correctness prerequisite for the tool-selection arc |
