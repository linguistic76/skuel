...
|---------|-----|-------------|--------|
| Semantic Analysis — SHIPPED #598–#600 (chips / prereq suggestions / ZPD feed); residue: ZPD semantic pool expansion | `docs/roadmap/done/SEMANTIC_ANALYSIS_ROADMAP.md` | Engagement edges exist (entry-enrichment arc) | small |
...
- **Tier 3 item 10 CLOSED BY DELETION** — `generate_facet_counts_query()`'s fragile `base_query.split("RETURN")[0]` string-split is gone: the whole `query_builders/` package was deleted (Aug 2026, PR #1081) after an audit found zero production invocations since 2026-05-12. Live faceting never used it — it composes facet counts in Python via `build_facet_counts` over `faceted_search_raw` results. **Forecloses nothing worth keeping:** if corpus-wide Cypher facet counts are ever wanted, build them fresh from `faceted_search_raw`'s structured parts rather than re-parsing rendered Cypher — that was the fix this item asked for, and it never has to be retrofitted onto dead code.
