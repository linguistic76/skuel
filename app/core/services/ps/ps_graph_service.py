...
class PsGraphService:
    ...
    async def find_time_aware_learning_path(
        ...
        Args:
            target_uid: Target path step UID to reach
        ...
        if not target_result.is_ok or not target_result.value:
            return Result.fail(Errors.not_found(resource="Path step", identifier=target_uid))
        ...
        results = await self.repo.find_time_aware_paths(
            target_uid=target_uid,
