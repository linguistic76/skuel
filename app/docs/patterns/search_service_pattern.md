...
class DomainSearchOperations(Protocol[T]):
    ...
    async def get_overdue(self, user_uid: UserUID | None = None, limit: int = 100) -> Result[list[T]]: ...
    async def get_active(self, user_uid: UserUID, limit: int = 100) -> Result[list[T]]: ...
```
...
**Temporal queries (get_upcoming / get_overdue / get_active):** The base `TimeQueryMixin` implementation works for most domains via `DomainConfig` fields — `date_field` (the column driving upcoming/overdue), `temporal_exclude_statuses` (defaults to 4 terminal statuses), `temporal_secondary_sort` (e.g., `"start_time"` for Events), and `completed_statuses` (defaults to `("completed",)`, Goals extend to `("completed", "cancelled")`). Only override when the domain has fundamentally different semantics (Habits use frequency windows computed from `last_completed` + `recurrence_pattern`; Principles express "overdue" as "past the 90-day review threshold" and "active" via the `is_active` flag).
...
class PrinciplesSearchService(BaseService[PrinciplesOperations, Principle]):
    ...
    # Harmonized time-query surface — overridden because Principles use is_active + 90-day review threshold
...
class PsSearchService(BaseService["PsOperations", PathStep]):
    """Search service for Path Steps (BaseService pattern)."""
