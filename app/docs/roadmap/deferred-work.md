...
| Dead-doc-links scanner narrowings — **3 measured items, scheduled 2026-09-02** (the placeholder guard is consulted by only one of four passes; a vocabulary gap for documentation metavariables; residual generic-subscript link targets) | Mike schedules it — approved work, not waiting on data. Cheap and self-contained; a natural ride-along on the next touch of `scripts/health/dead_doc_links.py` | Spec in [`dead-doc-links-sweep-queue.md`](dead-doc-links-sweep-queue.md) § Scheduled: three scanner narrowings — each with its measured shape, effect (−13 together) and its collision check against the tracked tree. ⚠️ Item 1 is a precondition for five of item 2's eight; one target is deliberately left reported rather than earning a one-off rule; and the obvious wider rule (reject uppercase stems) was measured and REJECTED — it swallows real doc names. Re-derive the counts before building |
