...
async def test_complete_semantic_search_flow(
    ...
    1. Prepare KU data (sync — ingestion never embeds inline, ADR-074)
    2. Generate the embedding via the embeddings service (worker-path recipe)
    3. Store in Neo4j
    4. Search for similar content and validate results
    """

    # 1. Prepare KU data — preparation is embedding-free by design (ADR-074):
    # the embedding is generated by the background worker from a post-persist
    # *EmbeddingRequested event, never inline during preparation.
    ku_data = {
    ...
    assert "embedding" not in prepared  # preparation never embeds (ADR-074)

    # 2. Generate the embedding the way the worker does: one text recipe
    embedding_text = build_embedding_text(EntityType.PATH_STEP, prepared)
