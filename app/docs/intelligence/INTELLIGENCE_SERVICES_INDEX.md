...
```python
# Each intelligence service wires a GraphContextLoader via the BaseAnalyticsService helper
...
# Services just inherit — no per-service override.
```
...
- `/core/services/llm_caller.py` - UnifiedLLMCaller (routes to OpenAI/Anthropic by model prefix; used by JournalOutputService + ExerciseReportService)
