...
- **GradeBook** (`/gradebook`) — 5 blocks: My Submissions (`/gradebook/mysubmissions`), Submit (`/submit`, cross-link to Workbench), Exercise Reports (`/exercise-reports`), Activity Reports (`/activity-reports`), Revisions (`/revised-exercises`). Hub view in `ui/gradebook/hub.py`, sidebar nav in `ui/gradebook/nav.py`.
