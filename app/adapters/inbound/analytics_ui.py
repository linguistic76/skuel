...
def create_analytics_ui_routes(
    ...
    @ui_boundary_handler("Error generating analytics")
    async def view_analytics(request: Request) -> Any:
        ...
        if params.period == "week_current":
            ...
            result = await analytics_service.generate_report(user_uid, analytics_domain, start, end)
        else:
    ...
    @ui_boundary_handler("Error loading Life Path alignment")
    ...
    @ui_boundary_handler("Error loading weekly life summary")
