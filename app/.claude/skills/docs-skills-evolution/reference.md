# docs-skills-evolution Reference: Evolution Workflows

> On-demand reference for the [`docs-skills-evolution`](SKILL.md) skill. SKILL.md holds the ecosystem-alignment philosophy, the two drivers of doc evolution, and the quick-reference file/command tables; this file holds the eight detailed workflows (library upgrade, pattern deprecation, stale-document audit, new-feature docs, cross-reference system, validation & tooling, validation checklist, ADR creation, fundamentals vs adaptive).
...
`dead_doc_links.py` replaces the manual `rg "DELETED_DOC_NAME" docs/` pattern. It catches four reference kinds at once (markdown links, backtick paths, bare absolute paths, and path tokens inside fenced code blocks) across the whole `docs/` and `.claude/skills/` tree. Five classes are deliberately excluded and **every one prints its count on every run** — freeform notes and templates (scope carve-out), history directories (`docs/migrations/`, `docs/roadmap/done/`, `docs/investigations/`, `docs/Reviews/`, where a dead link is a dated record being faithful), link targets that match the runtime route table (`scripts/health/route_catalog.py` — the real route tree wired onto a bare app, factory routes included), `<!-- historical -->`-marked citations inside `docs/decisions/`, and `<!-- planned -->`-marked ones inside live `docs/roadmap/` (files a plan intends to create; the marker self-retires when the file is built); see `docs/tools/HEALTH_CHECKS.md`. When `docs/INDEX.md` has broken links, it calls it out specifically:
