...
| `/adapters/persistence/neo4j/backends/` | 27 domain backend subclasses across 9 cluster files (`activity_backends.py`, `curriculum_backends.py`, `exercise_backends.py`, `user_entry_backend.py`, `sharing_backend.py`, `forms_backends.py`, `templates_backends.py`, `collab_backends.py`, `misc_backends.py`). Import directly from the cluster file, e.g. `from adapters.persistence.neo4j.backends.activity_backends import TasksBackend`. |
