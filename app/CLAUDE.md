...
- **Regular user navbar** icon links (left section, in order): **Explore** (`/explore`) + **Library** (`/library`) + **GradeBook** (`/gradebook`) + **Workbench** (`/workbench`) + avatar **U** (click → Profile + 6 Activity Domain links + Sign out dropdown). Right section: **Search** (`/search`) icon + notification bell.
