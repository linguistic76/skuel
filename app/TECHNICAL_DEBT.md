...
| 10 | `adapters/persistence/neo4j/query_builders/faceted_query_builder.py` | 211 | [ENHANCEMENT] | `generate_facet_counts_query()` extracts the MATCH clause by string-splitting a *generated Cypher string* (`base_query.split("RETURN")[0]`) — fragile if the base query's MATCH ever contains `RETURN` in a literal. Replace with structured query composition: build the facet-count query from the same structured parts as the base query rather than re-parsing rendered Cypher. (This path is test-only today — live faceting uses `build_facet_counts` over results.) Current split works for well-formed generated queries. | This facet-count path goes live, OR an observed base query breaks the naive `RETURN` split |
...
|---------|-----|-------------|--------|
| Semantic Analysis — SHIPPED #598–#600 (chips / prereq suggestions / ZPD feed); residue: ZPD semantic pool expansion | `docs/roadmap/SEMANTIC_ANALYSIS_ROADMAP.md` | Engagement edges exist (entry-enrichment arc) | small |
| Discovery Analytics Phases 2+ (Phase 1 logging + `/admin/analytics` gap surface shipped 2026-07-10) | `docs/roadmap/DISCOVERY_ANALYTICS_ROADMAP.md` | 1000+ `:SearchEvent` nodes (accumulating) | 2-3 days |
