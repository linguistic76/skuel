...
**One more trap worth naming, because it survived two proofs:** the block offsets are computed on the **raw** docstring, not `ast.get_docstring()`'s cleaned output. `clean=True` runs `inspect.cleandoc`, which drops the leading blank line of a docstring whose `"""` sits on its own line — so a cleaned offset added to the string node's `lineno` reports **one line early**, onto the blank line above the query. Two of the three real sites open that way, and neither the unit test nor the CLI proof caught it because both fixtures put the summary on the opening line. Raw offsets are correct by construction: raw offset *i* is source line `lineno + i`. `::test_a_newline_opening_real_docstring_anchors_to_its_query_line` now pins it against the real file.
