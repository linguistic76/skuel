...
- **Regular user navbar** icon links (left section, in order): **Activity** (`/activities`, icon `activity`) + **Explore** (`/explore`) + **GradeBook** (`/gradebook`) + **Library** (`/library`) + avatar **U** (click → Profile + Search + Sign out dropdown). Activity domain dropdown removed (2026-04-03) — domains accessed via `/activities` sidebar.
