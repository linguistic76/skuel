...
@pytest.mark.asyncio
async def test_intelligent_search_extracts_strength(search_service, mock_backend):
    mock_backend.find_by.return_value = Result.ok([])

    await search_service.intelligent_search("core principles")
...
@pytest.mark.asyncio
async def test_intelligent_search_extracts_category(search_service, mock_backend):
    mock_backend.find_by.return_value = Result.ok([])

    await search_service.intelligent_search("health principles")
...
@pytest.mark.asyncio
async def test_intelligent_search_active_filter(search_service, mock_backend):
    mock_backend.find_by.return_value = Result.ok([])

    await search_service.intelligent_search("active principles")
...
@pytest.mark.asyncio
async def test_intelligent_search_review_postfilter(search_service, mock_backend):
    """'review' keyword post-filters to principles with needs_review()==True."""
    recent = _principle(
        "p:recent",
    ...
    mock_backend.find_by.return_value = Result.ok(
        [recent.to_dto().to_dict(), overdue.to_dto().to_dict()]
    )

    result = await search_service.intelligent_search("core principles review")

    assert result.is_ok
    principles, _parsed = result.value
    uids = {p.uid for p in principles}
    assert "p:overdue" in uids
    assert "p:recent" not in uids
