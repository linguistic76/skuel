...
- **Activity Domains** — `render_activity_sidebar_page()` from `ui/activities/nav.py` — 8 items (Today, Tasks, Events/Calendar, Goals, Habits, Principles, Choices, Journal). Used on `/tasks`, `/goals`, `/habits`, `/choices`, `/principles`, `/journals`. "Events" sidebar link → `/cal` (redirects to current month); the calendar month/week pages render through the same `render_activity_sidebar_page()` (their Weekly/Monthly rows lit, `content_max_width="max-w-none"`), each showing its declared membership (`VIEW_SPECS`: month = events; week = events, habits, goal milestones and high-priority tasks, with the kind legend as filter). Activity Domains content is embedded inline in `/profile`.
