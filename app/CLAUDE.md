...
**Shared Components:** `EmptyState` (empty lists — adopted across ~45 locations), `EntityCard` (universal entity display with variant system), `StatsGrid`/`StatItem` (statistics grids), `render_error_banner`/`render_inline_error` (accessible error states — adopted across 20+ route files). All in `/ui/patterns/`.
