...
**Service:** `from core.services.sharing import UnifiedSharingService` — entity-agnostic; live: `share()`, `share_with_group()`, `submit_to_group()` (returns `created`), `get_shared_with_me()`, the per-group reads. There is no access check: a link grants what its reader reads, and an EntryReport is an owner read (`EntryReportService.get_for_user`, ADR-088 §3). The revoke / access-list / `set_visibility()` half has no door — PLANNED as operations on the edges audience-at-submit wrote, never a second share form: `/docs/roadmap/sharing-http-door.md`.
...
**Schema-change monitoring (opt-in, default OFF):** `SchemaChangeDetector` fingerprints the live schema, logs drift and keeps a migration history — that is its whole job: the cache-invalidating handler died with `query_builders/` (#1081) and the consumer-less handler seam was deleted 2026-08-29. On-demand via `Neo4jAdapter.check_schema_changes()`; or wire a background poll at startup with `NEO4J_SCHEMA_MONITORING=true` (+ `NEO4J_SCHEMA_MONITORING_INTERVAL`, default 900s, validated ≥1). Tier-independent (not `INTELLIGENCE_TIER`-gated) — off by default adds no always-on worker to CORE (the tier's guarantee is AI-scoped — the 5-min graph-health poller (`update_graph_health_metrics`, `scripts/dev/bootstrap.py`) is a CORE Analog worker; see `GRACEFUL_DEGRADATION_ARCHITECTURE.md`). **See:** neo4j-cypher-patterns skill § 7.

**Server tuning (memory, JVM, Vector API — sandbox/self-host only; AuraDB manages its own):** all server config is `NEO4J_*` env vars on the `neo4j` service in the base compose `../infrastructure/docker-compose.yml` (repo root; the app `docker-compose.yml` extends it, overriding only deltas). The Java Vector API (SIMD) is enabled via `NEO4J_server_jvm_additional=--add-modules jdk.incubator.vector` — required for optimal performance of the vector indexes (boot and `scripts/create_vector_indexes.py` both sync `EmbeddingGeometry.INDEX_LABELS` — one constant, not two lists); `2026.x` warns without it.
...
Every status-bearing write in `core/services/` goes through `backend.update_with_status_guard(uid, updates, guard)` — one primitive on `_CrudMixin`, declared on `CrudOperations[T]`. The statement takes the node's write-lock BEFORE reading the prior status, applies the guard's prior-conditional patches, and **returns the prior**; services derive every verdict (`is_completion_transition` / `is_reopen_transition`) from that returned prior.

- **A patch derived from state around the node** (a goal's linked-task tally) uses `_recompute_with_status_guard`: the same statement, run after a read taken under the node's lock in one transaction. Never count first and guard-write after.
...
- **Human-initiated per event** (ADR-070 Decision 9 — no background watcher). Three doors: the personal-vault "Sync" button, the admin "Sync content vault" button (`POST /api/vault/sync/content`, the one directory-ingest path), and one-shot `./dev vault-sync` (`scripts/vault_bridge_sync.py`, in-process reconciler).
- **Deletions propagate** on incremental/smart runs (entity file deleted → entity deleted; Edge YAML deleted → relationship deleted; move/rename + mass-deletion guards). A target dropped from a registered frontmatter field loses its edge on the file's next ingest (both doors) — only edges the file itself authored are retracted, diffed from the `authored_edges` fingerprint on its tracker row. A 🆔 task line deleted from a *surviving* note retires its `EXTRACTED_FROM` edge on the file's re-ingest (extraction pre-pass; gone = 🆔 absent AND digest on no 🆔-less line — a stripped token is re-minted, not retired); **the task stays, stamped** (`retired_vault_id` — the one-sync grace): a 🆔 that reappears within one sync re-links, an OPEN task whose line stays gone is CANCELLED by the next sync's sweep, a terminal one is untouched (ADR-070 Decision 1, R4). **See:** `/docs/patterns/UNIFIED_INGESTION_GUIDE.md` § Line-level deletion.
...
**API:** `POST /api/vault/sync` (personal), `POST /api/vault/sync/content` (admin), `POST /api/vault/preview` — the reconciler is the ONE ingestion system; there is no per-file or per-manifest HTTP door (ADR-070 Decision 9, amended 2026-09-22)
...
Bidirectional sync between a user's personal Obsidian vault and SKUEL. Tasks written to Obsidian as `- [ ] task title 🆔 sk_<6>`; task checkbox state flows **both ways** — outbound, SKUEL writes `[x]` + `✅ date` on completion **and un-checks the line + strips the `✅ date` on a reopen** (byte-exact reverse, gated on a TRAILING `✅` token so it takes back only SKUEL's own write; ADR-070 RDQ-2 amended 2026-08-24); inbound, a vault-side check, uncheck or field edit (title, 📅, ⏳, priority, `#tags`) of a 🆔 line reaches the task on the note's ingest — the extraction identity branch reconciles the line **three-way against the base its edge holds** (`EXTRACTED_FROM.source_line`, the line as SKUEL last saw it; `core/services/dsl/line_reconciliation.py`), so a field the vault changed is applied and a field it left alone keeps SKUEL's value; the write goes through `update_task`, and the base advances only when it lands (and with SKUEL's own outbound writes, applied to the base as to the file) — a refused edit re-warns every sync until line and task agree (ADR-070 Decision 3). **Moves re-point:** a 🆔 line cut into another note keeps its task — the edge moves in one statement, base intact. **Deletion cancels:** a 🆔 gone for two consecutive syncs cancels an OPEN task and leaves a terminal one (the retiring write stamps the task for a one-sync grace; re-type the line before the next sync to keep it; a whole-note deletion — or a note moved into a walled folder — is the same after the same grace); the end-of-sync sweep posts `CANCELLED` through the Tasks facade, clears the stamp only on ok, counts `VaultSyncStats.tasks_cancelled_by_deletion`, and **holds over any incomplete inbound pass** — a failed, unreadable or stale file, or a vault that read as empty or wiped (`inbound_pass_incomplete`). Both outbound writes are driven by the sync's **state predicate**, never by `TaskCompleted`/`TaskReopened` — a status transition is consumed by the write that produced it and has no retry. The `🆔 sk_<6>` suffix is the join key — never strip it.
...
**See:** `/docs/decisions/ADR-070-bidirectional-vault-bridge.md` (Decisions 1–3: base, stamps, the three-way merge, the sweep), `/docs/roadmap/done/r4-vault-inbound-propagation.md` (the R4 record — constraints, rulings, residuals), `/docs/decisions/ADR-075-local-agent-vault-transport.md`
...
One object (~250 fields), built by one concurrent round-trip — the MEGA-QUERY is six plan-cached statements, one per read family (`RICH_CONTEXT_STATEMENTS` in `user_context_queries.py`: tasks & goals, habits & events, principles & choices, knowledge, curriculum, learner state), merged by `execute_mega_query` into one map, plus the statements beside it (`SUBMISSION_STATS_QUERY`, `ENTRY_KNOWLEDGE_APPLIED_QUERY`, path steps, engagements, groups), all under one `asyncio.gather` — consumed by all intelligence services. Carries core identity from the `User` model (`user_uid`, `username`, `display_name`, `email`, `user_role`) — only fetch `User` directly when you need `user.preferences`. **A new read is a new registry entry, never a section appended to an existing statement:** the server serves a statement from its plan cache only up to a size (past it, every execution re-plans at ~0.5–1 s) and the planner's cost is super-linear in size, so the cold build is set by the largest statement; `tests/integration/test_user_context_plan_cache.py` derives its parametrization from the registry and measures the edge rather than counting lines.
...
- **Request Bodies (POST):** Pydantic request models — `parse_body(request, Model)` at a door both API clients and HTMX forms reach (JSON or a form encoding by `Content-Type`, an empty body as `{}`, one validator; the CRUD factory's create/update, the admin account actions), `parse_json_body` / `parse_form_body` where only one caller kind exists, or `body: Model` in the signature (FastHTML binds it during parameter extraction, *before* the handler and `@boundary_handler` run, so `install_request_validation_guard` is what converts a rejected model; ⚠ never annotate an auto-bound body field as a `Literal` — FastHTML calls the annotation to coerce and `Literal(...)` raises `TypeError`, which no guard converts). htmx never JSON-encodes a body — an `hx_headers` Content-Type override does not change what it sends.
...
- Query parameters for CRUD-factory reads (`/api/tasks/get?uid=...`); a path uid for a per-entity door, read or write (`GET /api/path-steps/{uid}/organizers`, `POST /api/tasks/{uid}/status`)
...
⚠ **Upgrading Alpine/HTMX touches three things:** the version constant in `ui/theme.py`, the vendored file under `static/vendor/`, **and** the versioned filename in `static/service-worker.js`'s `PRECACHE_URLS` — plus a `CACHE_VERSION` bump. `install` calls `cache.addAll()`, which rejects wholesale on a single 404, so a missed precache entry breaks service-worker install for every PWA client. The first three are pinned to each other by `tests/unit/ui/test_vendored_asset_pins.py`; the `CACHE_VERSION` bump is still a discipline.
...
| SKUEL035 | `Request` in `adapters/inbound/` comes from `adapters.inbound.fasthtml_types` — never `fasthtml.common` / `fasthtml.core` / `starlette.requests` (the same class through one door; the re-export module itself is exempt) | ERROR |
| SKUEL036 | No `require_authenticated_user()` inside a `@require_admin` / `@require_teacher` / `@require_role` handler — the decorator authenticated and injected the caller: `UserUID(current_user.uid)` (AUTH_PATTERNS § Pattern 3; ungated handlers are Pattern 2 and untouched) | ERROR |
...
- Ruff and Black `target-version` are both `py314`. **TC002/TC003 are ignored permanently** (ruled 2026-08-28 — a TYPE_CHECKING move breaks any annotation evaluated at runtime, and FastHTML handler signatures are); **UP037 is live** (tree swept 2026-09-16 — a quoted annotation is now a lint error, and a `NameError` at bootstrap on a `TYPE_CHECKING`-only name is fixed with a runtime import, never by re-ignoring the rule). Rationale in ADR-067 § Deferred.
...
- **Codex** is on-demand via `scripts/request_codex_review.sh <PR#>` — run it AFTER the final push (the gate drops the label on synchronize). The script polls patiently in-bash (Codex can take >13 min — #317) and **never labels without a real verdict**: on timeout the gate stays RED (exit 3) and proceeding past a genuine outage is a deliberate human call, not a timer. Exit codes: 0 clean+labeled / 1 operational failure (read the output — an unreadable window means no verdict, a label failure after a clean one means only re-label) / 2 findings-read / 3 timeout. **On this laptop wait in bounded FOREGROUND calls** (`<PR#> 540`, under the 10-min tool cap — the harness's low-memory guard kills background waiters on desktop spikes) **and continue with `<PR#> 540 --resume`**, which posts no second summon; a plain re-run does, and anchors past any verdict in between.
