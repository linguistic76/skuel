...
| File | Responsibility |
|------|---------------|
| `goals_intelligence_service.py` | Shell: `__init__` + dataclasses (`GoalPrediction`, `HabitImpactAnalysis`); label attrs resolved via `DomainConfig`; `analyze_learning_patterns()` — delegates to `KnowledgePatternAnalyzer` (June 2026, #368). Route: `GET /api/goals/knowledge-patterns` |
