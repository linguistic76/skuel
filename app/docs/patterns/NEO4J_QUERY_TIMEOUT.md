...
- **Tests:** `tests/unit/test_timed_driver.py` (12 tests — fake-session captures the `Query` object), `tests/integration/test_timed_driver.py` (5 tests against the pinned calendar-release testcontainer — tag read from `infrastructure/docker-compose.yml`; busy Cartesian-product Cypher — no APOC, per SKUEL001).
