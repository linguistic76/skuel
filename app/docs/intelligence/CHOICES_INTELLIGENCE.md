...
| File | Responsibility |
|------|---------------|
| `choices_intelligence_service.py` | Shell: `__init__` + 3 protocol delegation methods (label attrs resolved via `DomainConfig`); `analyze_learning_patterns()` — delegates to `KnowledgePatternAnalyzer` (June 2026, #368). Route: `GET /api/choices/knowledge-patterns` |
