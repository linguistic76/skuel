...
- **Tests:** `tests/unit/test_timed_driver.py` (12 tests — fake-session captures the `Query` object), `tests/integration/test_timed_driver.py` (5 tests against `neo4j:5.26.0` testcontainer; busy Cartesian-product Cypher — no APOC, per SKUEL001).
...
- [`@neo4j-cypher-patterns` skill — "Per-query timeout"](../../.claude/skills/neo4j-cypher-patterns/PATTERNS.md) — actionable how-to for query authors.
