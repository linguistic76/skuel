...
| D3 | ✅ **RESOLVED at the write (2026-09-04).** Was: ingested tasks invisible to the goal Gantt — `get_tasks_for_goal` reads the node *property* `fulfills_goal_uid` and the ingestion door wrote only the `FULFILLS_GOAL` *edge*. Every writer now lands both halves: the app doors dual-write property + edge (`TasksCoreService._write_link_edges`, `TasksService._sync_relationship_edges`), the vault door stamps the property from `connections.fulfills_goal` (`core/services/ingestion/preparer.py`). Invariant: property == edge target. The 2 pre-existing vault rows (`task.three-moments-values`, `task.track-one-pattern`) gained the property on the 2026-09-04 force content re-sync (`./dev vault-sync --force --vault content`), proven live: both columns equal their edge target and no task carries a column without a matching edge. | `tasks_search_service.py:84` | Done at the write, not the read; `test_vault_task_goal_link_parity.py` pins the vault half. |
