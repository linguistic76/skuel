...
| [Askesis Intent Classification — Activation Arc](roadmap/askesis-intent-classification-activation.md) | Scheduled 2-PR arc to turn on the intent-conditioned Askesis layer, which has never executed: the 0.65 gate is a MEAN over 8 exemplars and is unreachable. Activates the two answer-shaping branches (graph context, suggested actions); the chunk-type filter stays off as a gated PR-3. Retires AGGREGATION's exemplars (the enum member stays — the tool-selection sketch depends on it) |
