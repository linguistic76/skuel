...
- `Pipeline.allows_sharing()` returns `False` for `REFERENCE` and `TRANSCRIBE_AND_STRUCTURE`. No audience picker is offered in the journals UI, and a saved chat is owner-private with no sharing surface at all (ADR-078). ⚠️ The pipeline guarantee rides on the **pipeline**, so it does not extend to a note stored under a shareable one — `extract_activities` and `knowledge` permit sharing, and the vault/YAML door **defaults an absent `audience:` to `teachers`** — expanded to every group the user is a student-member of, private only when there are none (`AudienceResolver.resolve_default_teachers`). A context note a student wants kept to themselves must author `audience: private`.
- A saved chat is user-owned only: no teacher visibility, no group sharing, no admin read access through normal domain APIs. That guarantee is the chat's, not the synced note's — see the audience default above.
