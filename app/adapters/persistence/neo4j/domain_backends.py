...
class LpBackend(UniversalNeo4jBackend[LearningPath]):
    ...
    async def persist_path_with_steps(
        ...
        CREATE (u)-[:HAS_PATH]->(p)
        """
        path_result = await self.execute_query(
            path_query, {"user_uid": user_uid, **path_params}
        )
        ...
        self.logger.debug(
            f"Persisted path {path_params['uid']} with {len(steps_params)} steps"
        )
        return Result.ok(True)
    ...
    async def identify_path_blockers(
        ...
        """
        return await self.execute_query(
            query, {"path_uid": path_uid, "user_uid": user_uid}
        )
    ...
    async def get_path_with_context(
        ...
        {self._build_prerequisite_subquery("k", depth)}
