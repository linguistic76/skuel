...
| **Vault sync privacy wall** | Fail-closed folder allowlist (`SyncAllowlist` / `SKUEL_VAULT_SYNC_ALLOWED_DIRS`): only allowed folders under the personal vault root are ingested (`periodic_notes/` by default when unset); everything else is walled off. On by default — does not depend on config. Enforced on every path: directory scans (`collect_files`) **and** single-file ingest (`ingest_file`), so both `/api/ingest/directory`, `/api/ingest/file`, and the reconciler inherit it | Active (default-on) |
