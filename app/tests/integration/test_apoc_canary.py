...
class TestNeo4jVersionCanary:
    ...
    The server tracks the calendar line (2026.04.0); the driver stays on the
    ...
    async def test_neo4j_server_version_matches_calendar_pin(self, neo4j_driver: AsyncDriver):
        """
        Canary: Verify the Neo4j server is the pinned calendar release (2026.04.0).
        ...
        async with neo4j_driver.session() as session:
            ...
            assert version.startswith("2026.04"), (
                f"Expected Neo4j 2026.04.x, got {version}. "
