...
| No HTTP route hands Cypher a caller-supplied label, property name or relationship type | Every such name reaching a `build_*` function passes `validate_label()` / `validate_identifier()` (`query/cypher/_helpers.py` — the same pair `neo4j_schema_manager`'s DDL imports) or the builder's model-field check; `ModelQueryBuilder.filter` / `order_by` (`unified_query_builder.py`) → `validate_field_name`. **No route publishes a sort key at all** — `GET /api/{domain}/list` takes pagination only, because `ORDER BY` on a property the response never renders is a measured disclosure oracle. A handful of backend methods do interpolate a caller's property name unchecked (`_relationship_ordered_mixin`, `PsBackend.list_steps_raw`); each caller passes a literal, a registry constant, or sits behind a PLANNED surface — `/docs/roadmap/field-name-guarding-in-cypher.md` holds the ruling. Comparison operators and sort directions need no validator: operators are picked by structural dispatch (an if/elif chain, a guarded `op_map`, or `_FILTER_OP_MAP`), which emits a literal and cannot emit an unknown one; directions resolve to `"ASC"`/`"DESC"` from a bool, from `RelationshipSpec.order_direction`, or from a literal at the call site |
