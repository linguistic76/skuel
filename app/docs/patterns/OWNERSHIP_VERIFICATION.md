...
Every route factory that acts on a USER_OWNED entity by uid verifies ownership before it
...
Reads on behalf of a user pass exactly one of two enforcement chokepoints
...
1. **The visibility clause** — `build_search_visibility_clause()` composes the audience
   ...
   `BaseService.get_visible_to_user(uid, user_uid)`: it passes the domain's OWN
   `read_visibility` declaration (the search declaration unless declared apart — UserEntry
   opens for its audience, `OWNER_OR_AUDIENCE`, while searching owner-only; ADR-088 §5)
   to the backend and converts the None into the service-layer NotFound, so a caller can
   never pick a visibility literal at the call site.
...
```python
# core/services/mixins/crud_operations_mixin.py — CrudOperationsMixin, inherited by BaseService
...
async def update_for_user(self, uid: str, updates: U, user_uid: UserUID) -> Result[T]:
    """Update entity only if owned by user. `updates` is the domain's typed
    update value (a `TaskUpdateIntent`, or `RawChanges`) — never a bare dict."""
...
The shape to refuse in review is the same handler without the check — a uid taken from
the request and handed straight to the service acts for anyone who can guess it:

```python
@rt("/api/transcriptions/process", methods=["POST"])
async def process_transcription(request, uid: str):
    return await transcription_service.process(uid)  # anyone with the UID can act
```
...
- **Owner-bound curriculum** (Exercise) carries `owner_uid` plus the
  ...
  `create()` returns the failure when the edge write fails (ADR-086 door 4), so
...
| UserEntry | UserEntry | User's entries — journal, grounded knowledge, exercise turn-ins (ADR-054) |

### Shared Domains (scope=ContentScope.SHARED)

| Domain | Entity | Notes |
|--------|--------|-------|
| KU | Ku | Shared knowledge base |
| PS | PathStep | Shared learning content |
| LP | LearningPath | Shared curricula |

MOC is emergent identity: any Entity with outgoing `ORGANIZES` edges is one. There is
no MapOfContent entity type.
...
3. **A migration must never widen an existing audience.** Backfilling a default
   ...
   cannot leave a half-audienced entity that the narrow work list then misses.
...
| Domain | Entity | Notes |
|--------|--------|-------|
| Finance | Invoice (`InvoiceDTO`) | Admin-only — `@require_admin` on every `/api/invoices/*` and `/finance/invoices` route, no ownership checks; Firefly III is the ledger (ADR-052) |
...
`scope` is the factory's only ownership switch: `CRUDRouteFactory.verify_ownership` is
derived from it (`scope == ContentScope.USER_OWNED`), never passed.
...
async def test_ownership_prevents_cross_user_access():
    ...
    created = await service.create(Goal(user_uid=UserUID("user.alice"), title="Alice's Goal"))

    # User B tries to access it
    result = await service.get_for_user(created.value.uid, UserUID("user.bob"))
...
- `/core/services/mixins/crud_operations_mixin.py` - `verify_ownership` and the `*_for_user` methods every `BaseService` inherits
- `/adapters/inbound/route_factories/route_helpers.py` - `verify_entity_ownership` and `require_owned_entity`
