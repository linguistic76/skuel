...
**Detection (AST):** flags a function that (1) declares `**kwargs` (any name), (2) has **no** explicit `cls` parameter (positional, keyword-only, or positional-only), and (3) contains a call passing both a `cls=` keyword and that same `**kwargs`. A `kwargs.pop("cls", ...)` that runs **before** the splat is exempt — it removes `cls` from the mapping, so no collision. Two things are deliberately **not** exempt: `kwargs.get("cls")` (`get` reads but leaves `cls` in the mapping, so the splat still collides) and a splat that runs *before* the pop (e.g. on a guard-return path). The explicit `cls: str = ""` parameter is the preferred, self-documenting contract.
