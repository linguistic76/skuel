...
def create_tasks_ui_routes(
    ...
    async def task_edit_submit(request: Request) -> Any:
        ...
        # (an explicitly-cleared field clears; a field the form does not render is not
        # written). A cleared edge picker posts "", which parse_form_body maps to None —
        # the explicit-clear signal update_task turns into an edge deletion. Pinned by
