...
class OwnershipRouteFactory:
    ...
    def register_routes(self, _app: Any, rt: Any) -> None:
        ...
        for config in self.routes_config:
            self._register_route(rt, config)

        logger.info(
            f"Ownership routes registered for {self.domain}: {[r.path for r in self.routes_config]}"
        )

    def _register_route(self, rt: Any, config: OwnershipRoute) -> None:
