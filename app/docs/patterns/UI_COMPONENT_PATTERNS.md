...
- [/ui/activities/nav.py](/ui/activities/nav.py) - `render_activity_sidebar_page()`, the sidebar page wrapper every activity domain page, the calendar views and Today use — one list (`ACTIVITY_SIDEBAR_ITEMS`)
...
One navbar for every role, one rule: the chrome carries **one door per section**, lit for the whole section by a section key; a section's pages are its sidebar's rows, lit by slug. A door's href may sit in a sidebar only as its first row (Tasks+ → `/today`) — the one sanctioned overlap; Library and Submissions land outside their sidebars, PathSteps has none. `tests/unit/ui/test_navbar.py` pins it, including an anti-duplication invariant over every `*_SIDEBAR_ITEMS`.
...
**Teaching UI as canonical example:** `ui/teaching/forms.py` holds `render_feedback_submission_form()`, `render_revision_request_form()`, `render_waiting_actions()`, `render_submission_metadata()`, `render_form_responses_section()`. `teaching_ui.py` and `teaching_forms_ui.py` call these functions — they contain no inline `Form`/`Input`/`Label` construction. Dict→dataclass converters live in `ui/teaching/types.py`; submission bucketing lives in `TeacherOrchestrator.get_bucketed_student_submissions()`.
...
**System UI:** `ui/system/` — `landing.py` (login landing page with hero + form), `error_pages.py` (404 page). `system_ui.py` — `/` is a 303 to `/today` for every authenticated role, the landing for anyone else.
...
async def task_detail_view(request, uid: str) -> Any:
    ...
    if owned.is_error:
        # The banner the learner sees, at the status the failure earns:
        # 404 for a foreign or missing uid, the fault's own for a backend failure.
        return refuse(owned.expect_error(), render_error_banner, "Task")
    task = owned.value
    content = Div(
