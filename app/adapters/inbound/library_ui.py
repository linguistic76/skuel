...
def _exercise_item(uid: str, title: str, description: str | None) -> Div:
    """Single row for an exercise the user is registered with."""
...
def render_exercise_list(exercises: list[Any]) -> Div:
    """Render exercises the user is registered with (via group membership)."""
    if not exercises:
        return EmptyState(
...
def create_library_ui_routes(
    ...
    **_kwargs: Any,
    ...
    async def library_exercises(request: Request) -> Any:
        """HTMX fragment: exercises the user is registered with via group membership."""
        user_uid = require_authenticated_user(request)

        if not exercises_service:
            return render_error_banner("Exercise service unavailable")

        result = await exercises_service.get_student_exercises(user_uid)
        if result.is_error:
