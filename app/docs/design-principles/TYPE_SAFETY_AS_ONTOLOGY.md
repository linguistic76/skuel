...
In a system with 22 entity types, 14 statuses, 4 roles, and 6 lateral relationship types, raw strings create invisible coupling. `entity_type == "task"` compiles fine even when `EntityType.TASK` has been renamed or removed. `entity_type == EntityType.TASK` fails immediately at the type checker, before any code runs.
