...
**Pattern:** `ui/` is pure presentation — it renders what routes hand it. The dependency arrows point inward: `adapters/inbound` (routes) imports `ui/` components, never the reverse at runtime. This is SKUEL022's sibling for the `ui/` layer; both share the same AST scan (`_collect_runtime_layer_imports`, parameterised by target package — SKUEL032 is the third caller). Before this rule, no lint watched `ui/`, which is how the former `converters.py` under `ui/calendar/` grew an adapters import (fixed in #653; the module itself was deleted in #917) and CSRF render helpers were consumed from `adapters/inbound/csrf.py` (split inward in #654). The rule replaced (and #656 deleted) the interim unit-test guard `test_ui_layer_boundary.py` (one enforcement point — lint fires in `./dev quality`, not only under pytest; inline suppressions beat a remote sanction list).
...
**Known and accepted miss:** a loop variable *embedded* in a comprehension element, as in `str([(u, title) for u in entity_uids])`. Only the identity form traces back to the iterable; catching the embedded one means binding `u` to its iterable and following that alias through arbitrary nesting — variable tracking rather than a shape test, which this linter declines by ruling. The failure direction is a **miss**, the safe one for an ERROR gate: the alternative over-approximates, and a false positive blocks CI on correct code.

**The standing rule for this rule's future**, distilled from ~20 review findings on #1194: a *false positive* is a defect to fix, because it blocks CI on correct code; a *miss* of a shape that would need variable tracking is a documented limit. SKUEL034 guards the shape with no legitimate form — it is not a proof that no uid ever reaches an `in`.
...
**Pattern:** any `from <module> import … Request …` in `adapters/inbound/` where the module is not `adapters.inbound.fasthtml_types` — `fasthtml.common`, `fasthtml.core`, `starlette.requests` — and any attribute-qualified `<base>.Request` whose base is not the door (`starlette.requests.Request` after `import starlette.requests`, `fh.Request` after `import fasthtml.common as fh`). The door is resolved through the file's own imports — a qualifier is the door only when it is bound by `import adapters.inbound.fasthtml_types [as x]` or `from adapters.inbound import fasthtml_types [as x]` — never by spelling, so `import starlette.requests as fasthtml_types` does not pass. A parenthesised import is reported at the `Request` alias's own line, which is where a line suppression is written. A quoted annotation (`"starlette.requests.Request"` — SKUEL020 accepts it, and ruff's UP037 leaves a *dotted* quoted annotation alone) is parsed and walked exactly like the bare expression. Single-line and parenthesised multi-line imports alike (AST, so the multi-line form a line-regex misses is covered); the attribute half exists because SKUEL020 accepts the fully-qualified Starlette annotation, so a module import would otherwise pass both rules. Only the name `Request`; `Response`, `JSONResponse` and the FT tags keep their sources.
...
**Guard test:** `tests/unit/scripts/test_lint_skuel.py::TestSKUEL035` — the tag-adjacent single line, the parenthesised multi-line import, the bare Starlette import, the module-import-plus-qualified-annotation bypass and its aliased form, a Starlette import aliased to the door's name, quoted annotations (bypassing and clean), the clean door (bare, qualified, and aliased three ways), the alias-line report and suppression of a parenthesised import, other names from `fasthtml`, the re-export module's exemption, the out-of-scope `ui/` tree, and line suppression.
...
**Scope:** `adapters/inbound/`. Both names are resolved through the file's `from … import … as …` bindings — `require_role as gated` is still the gate, `require_authenticated_user as authenticate` still the call, and a local name an import bound to something else is not either. A handler with no role decorator is Pattern 2 — `require_authenticated_user` is *its* one path — and is untouched; so is `roles.py`'s own wrapper, which calls it by design and carries no role decorator.

**Fix:** `user_uid = UserUID(current_user.uid)`, or delete the call when its value was unused.

**Guard test:** `tests/unit/scripts/test_lint_skuel.py::TestSKUEL036` — the assigned and bare calls, a nested factory handler, both names under import aliases, a look-alike name bound elsewhere, the one-path positive, the ungated Pattern-2 handler, the decorator module itself, and line suppression.
