...
3. **Open the PR** against `main`: `gh pr create`. Write a real description — Kody generates no PR summary (`summary.generatePRSummary: false`), so your description is the PR's only account of the change.
...
5. **Summon a reviewer when ready.** Neither AI reviewer runs automatically (since 2026-05-24). Comment **`@kody start-review`** for Kody's gating review. For any PR touching Python files, also run **`scripts/request_codex_review.sh <PR#>`** — the Codex Review Gate is RED until you do. Do this once a PR is substantive, not on every intermediate commit. **Wait in bounded foreground calls, and resume rather than re-summon:** on the development laptop (15 GB RAM, a 2 GB swapfile) Claude Code's low-memory guard stops *background* commands on transient desktop spikes — measured six times in one session, each on a trivial waiter with 7–10 GB available a minute later — and a killed wait loses the poll, not the summon. So run the script as a **foreground** call with a deadline under the harness's 10-minute tool cap (`scripts/request_codex_review.sh <PR#> 540`), and when it exits 3, **`scripts/request_codex_review.sh <PR#> 540 --resume`** as many times as needed: `--resume` posts nothing: every summon the script posts records the head SHA it asks about, and a resumed wait anchors at the oldest summon by your account **for the current head** since the last `codex-considered` — so the verdict is read wherever it lands (after the summon, after the re-nudge or between the two), a summon for another head is never resumed whatever its date, and a summon posted by hand is not resumable (run plainly). It never re-nudges. A plain re-run posts a second summon and anchors at it, so a verdict landing between the two is never read.
...
| **Kody** (`kody-ai[bot]`, Kodus) | Semantic risks CI cannot prove — the v2 categories bug, security, performance and business logic at `high` severity and above, plus the dashboard Kody Rules — per `kodus-config.yml` (honoured only while the repo's `kodusConfigFileOverridesWebPreferences` is ON in app.kodus.io) | A check-run always; a real PR review **only when it has findings** — a clean run posts no review object, so its check-run concluding `success` IS the verdict signal (`./dev pre-merge` check 3 reads it) | **On demand** — `@kody start-review`, for the first review and every follow-up (auto-review turned off in the Kodus dashboard; the repo file agrees, `reviewCadence: manual`) | **Yes, when summoned** — `CHANGES_REQUESTED` holds the merge until resolved/dismissed |
...
- **Codex is the second opinion you summon.** Comment **`@codex review`** to invoke it. Its *verdict* is *advisory* and **invisible to `gh pr checks`** — what shows up in checks is the separate **Codex Review Gate** (above), which only enforces that you *considered* the review. The verdict can land on any of **three** surfaces, so scan all three (the first command below misses inline review-comments — the surface Codex often uses for a specific finding):
  ...
  **Six failure modes to rule out before you apply `codex-considered`:**
  ...
  4. **A review's BODY and its inline comments are separate surfaces — read both.** One review can carry findings in either, or only in its body, and a query that prints `.state` without `.body` shows a body-only finding as an empty `COMMENTED` review. Measured on #1301: 20 inline comments and **1 body-only P1** across 9 reviews (8 of which held nothing but boilerplate) — the body-only one was a real hole in the push-time secret scan, and it was found only by querying the API by hand after the round was reported complete. `scripts/request_codex_review.sh` prints both surfaces; a manual check must too. ⚠️ Related trap: GitHub **re-points an outdated inline comment to a newer SHA** when its line still resolves, so `commit_id` does not date a finding — `original_commit_id` does. A batch of "new" findings on the head SHA is often old ones resurfacing. For staleness, read **`subject_type`** first: a `"line"` comment with `line: null` is outdated (its line left the diff), but a `"file"` comment has `line: null` **by design** and may be current — treating every null `line` as stale would dismiss a live file-level finding, which is the expensive direction to be wrong in.
  5. **Codex's account of what it DID is not evidence that it did it.** Its summaries are written in the first person and can describe work that does not exist — observed on #1301: *"Committed the changes as `11ed274`… and created the required follow-up PR"*, where the object was unreachable in the repository and no such PR had been opened. The **finding** in such a summary may still be real and worth fixing; the **claim of authorship is not.** Verify with `git cat-file -t <sha>` and `gh pr list` before believing a fix has landed, and never close a finding on the strength of Codex saying it handled it. (`request_codex_review.sh` already refuses to auto-label on any comment shape but the known clean signature, so the script will not clear the gate on one of these — but a human reading the summary can.)
  6. **The script's exit code IS the verdict, and a pipe throws it away.** `request_codex_review.sh` exits **0** clean verdict read and labeled / **1** operational failure / **2** findings printed and read / **3** timeout, gate stays RED (keep waiting with `--resume`, never a plain re-run — see step 5). Only **0** means both halves happened, so the code is the difference between "clean" and "there are findings nobody has read". **Exit 1 is two different situations and the output says which:** a failed summon, a usage/auth error, an unreadable window, or `--resume` with no summon of yours on the PR means no verdict exists and the review must be (re-)summoned; a label that could not be applied *after* a clean verdict was read means the verdict is fine and only `apply_codex_considered.sh` needs re-running. So no code substitutes for reading the output — failure mode 1 again, arriving as a number. `script 2>&1 | tail -40; echo "EXIT=$?"` reports **`tail`'s** status, not the script's — observed on #1306, where a findings run (exit 2) surfaced as `exit code 0` and the findings were caught only by reading the log text. The same shape launders a command that never ran: `127` becomes `0`, and empty output plus a zero code reads exactly like a clean review. Run it bare, or redirect and record the code inside the log — `cmd > "$LOG" 2>&1; echo "REAL_EXIT=$?" >> "$LOG"` — never into a compound whose own status is what gets reported. Either way the verdict is confirmed by **reading the log's text**, per failure mode 1.
...
1. ~~**No PR template.**~~ *(Resolved 2026-05-22)* — `.github/pull_request_template.md` now exists, so descriptions are standardized — valuable because the human description is the PR record (Kody generates no summary).
