"""
Shelved stub routes from lesson_ui.py — 2026-03-29

These were placeholder routes with no real functionality.
...
# --- Stub route functions (were inside create_lesson_ui_routes) ---


def lesson_discovery_dashboard(_request) -> Any:
    """Discovery dashboard."""
    return Div(
...
def lesson_analytics_dashboard(_request) -> Any:
...
def lesson_graph_page(_request) -> Any:
...
def moc_nav_fragment(lesson_service):
    """HTMX: Load Maps of Content navigation list for sidebar."""

    async def _handler(request) -> Any:
...
def lesson_edit_form(_request, uid: str) -> Any:
...
def lesson_graph_view(_request, uid: str) -> Any:
...
def knowledge_javascript(_request) -> Any:
