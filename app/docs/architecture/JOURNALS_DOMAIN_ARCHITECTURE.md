...
**Persistence (ADR-073 + ADR-078, opt-in):** the **file/audio door** is fully **zero-persistence** — it writes the processed output to the user's own `je_out/` folder (see §7), writing nothing to Neo4j. Every discussion (both doors) is **ephemeral by default** (ADR-078 §5, founder realignment 2026-07-13): its memory lives in a client-side transcript accumulator that dies on reload, and it reaches the **owner-private `:ConversationSession` + turns** store **only when the user explicitly presses *Save this chat*** (`POST /journals/save`) — there is no auto-save. The store is **understanding-agnostic**: even a *saved* chat writes **nothing** to the understanding channel (no `UserEntry`, no embeddings, no search/context), so ADR-073's understanding wall (§2) holds intact. Saved discussions are revisitable/continuable via `/journals/discussion/{id}`; the user still keeps whatever they copy or download. (P3 PR1 shipped the typed door's ephemeral-default + Save; PR2 gives the file/audio door the same Save affordance — until then it keeps its own client-side accumulator.) Audio transcription is synchronous and file-based (decision A). Periodic notes remain a deliberate stored feature.
...
| Routes | `adapters/inbound/journals_routes.py` | FOUNDER enforcement (`journal_tier.is_founder()`) lives at the route layer. Text-entry path: `POST /journals/start` runs AI and returns the response **inline** (`HX-Retarget` `#journal-workspace`) — **ephemeral by default** (ADR-078 §5): it persists **nothing** (the opening pair rides the composer as a structured `transcript_json` accumulator), **no `UserEntry`**, no redirect. `POST /journals/follow-up` selects one of three memory models: session-backed (a *saved* chat — reads/appends turns from Neo4j, 404-not-403 on a non-owner), ephemeral-structured (the typed door's unsaved `transcript_json`, grown via OOB swap), or the flat legacy accumulator (file/audio + DNWF doors, until P3 PR2). `POST /journals/save` is the single explicit persistence gesture — folds the ephemeral transcript into an owner-private `:ConversationSession` + turn pairs (`ConversationService.save_transcript`), swaps the composer to its session-backed shape, and OOB-refreshes the revisit list. File-upload/`folder-process` paths process to the user's own `je_out/` folder via one shared stateless batch engine (also no `UserEntry`); FOUNDER audio → transcript review → Scribe. Response fragments carry a lazy-loaded "Suggested activities" panel (`POST /journals/suggest-activities`, FULL tier) that takes the reflection **content in the body** — no stored entry to read or cache. `GET /journals/{entry_uid}` is **periodic-notes-only** (daily/weekly/monthly); any non-periodic uid → 404. `GET /journals/je-out/{filename}` serves flat `je_out/` files (`.md`/`.txt`) with a path-containment guard. The **discussion revisit surface** (ADR-078) lives at `/journals/discussion/{session_id}` — GET continues (rehydrate workspace from stored turns + restored source selection), `…/delete` (POST) drops the session subtree, `…/export` (GET) streams a markdown transcript, `…/rename` (POST) re-titles it; the `/journals` sidebar lists the user's owned sessions. Every op is owner-scoped (404-not-403). |
