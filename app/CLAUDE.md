...
- **Regular user navbar** icon links (left section, in order): **Hub** (`/home`) + **Tasks+** (`/profile`) + **Explore** (`/explore`) + **Library** (`/library`) + avatar **U** (click → Profile + 6 Activity Domain links + Sign out dropdown). Right section: **Search** (`/search`) icon + notification bell. After login, regular users land on `/home`.
