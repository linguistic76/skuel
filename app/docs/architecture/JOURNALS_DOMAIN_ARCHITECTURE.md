...
- ⚠️ **`Pipeline.JOURNAL` is not the pipeline of every stored note in this domain.** It is **authored, never assigned**: no service sets it, and the vault/YAML door accepts it from frontmatter (`_parse_pipeline` admits every member but the two audio ones). The notes that surround journaling carry other pipelines by design — `ensure_periodic_note()` writes in-app periodic stubs as `Pipeline.NONE` (deliberately; see its docstring), a vault periodic note is authored `extract_activities`, and a consented `je_pro/` file stores **whatever pipeline its frontmatter authors** — `je_pro_skip_reason()` gates on a pipeline being *present*, not on which one, and the guidance message recommends `knowledge` without anything coercing it. Read the pipeline off the entry; never infer it from the domain or the folder.
- `Pipeline.JOURNAL.allows_sharing()` returns `False`, alongside `REFERENCE` and `TRANSCRIBE_AND_STRUCTURE`. No audience picker is offered in the UI; sharing cannot be unlocked through the API. ⚠️ That guarantee rides on the **pipeline**, so it does not extend to a note stored under a shareable one — `extract_activities` permits sharing.
