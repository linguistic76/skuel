...
- **Regular user navbar** icon links (left section, in order): **Hub** (`/home`) + **Tasks+** (`/profile`) + **Explore** (`/explore`) + **Library** (`/library`). Right section: **Search** (`/search`) icon + notification bell + **Sign out** (`/logout`) icon. After login, regular users land on `/home`.
