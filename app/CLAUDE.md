...
- **`UserEntryService.create_entry()` is the one convergence point; every door reaches it directly, with no shared middle layer** (ADR-054). Contrast the two that get confused: the exercise **turn-in** — `/submit` is a legacy 302 onto the canonical `/submissions/exercise`, whose form HTMX-posts to `POST /api/user-entries/upload`, and that handler in `adapters/inbound/user_entry_api.py` builds the request and calls `create_entry()` itself — versus the **vault/YAML** door, `ingest_user_entry` (`core/services/ingestion/user_entry_ingestion.py`), whose one caller is `UnifiedIngestionService` reaching it from `ingest_file()`, so unlike the turn-in it **is** on the directory-ingest path. Neither goes through the other. (Not an exhaustive list of callers: the sibling `/api/user-entries` and `/api/user-entries/form` routes, and the derived-child write in `UserEntryProcessingService`, also call `create_entry()` directly.) The vault is the source of truth for user data; `/settings/vault` (Obsidian bidirectional sync) is the primary personal-data path.
