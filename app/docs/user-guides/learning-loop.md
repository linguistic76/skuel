...
```
[Path Step] ──▶ Exercise → UserEntry → EntryReport → RevisedExercise → ...
```
...
1. **The curriculum** — what Path Steps exist, how they connect, what's prerequisite to what
...
You can talk with Askesis from any Path Step page, any Learning Path, or from the Askesis home page for open-ended dialogue.
...
1. **Discover** — You browse Path Steps in the Explore hub or follow a Learning Path. Askesis might suggest what to explore next based on your recent journals and activity.
...
A Learning Path is a structured sequence of Path Steps. Each step references specific curriculum content and Kus, and tracks your mastery independently.
...
**How paths connect to the loop:** Steps within a path link to Exercises. When you complete an Exercise's submission-feedback cycle, your mastery on the related step increases. The path's progress is the ratio of mastered steps to total steps.
...
2. Create Path Steps in sequence, each composing the Kus students should engage with
...
The path structure gives students a clear road through your curriculum while the loop (Exercise -> UserEntry -> EntryReport -> RevisedExercise) ensures they actually engage with each step's content.
...
1. **Create curriculum** — Write Path Steps that present knowledge. Create Exercises that operationalize that knowledge into concrete assignments.
...
3. **Review submissions** — Your review queue shows pending student work. For each submission, you can:
   - Write feedback (creates a EntryReport record)
...
├─────────────────────────────────────────┤
│  1. Learning Loop (base)                │  Exercise → UserEntry → EntryReport → ...
└─────────────────────────────────────────┘
```
...
| **Revision chain** | The sequence Exercise → UserEntry → EntryReport → RevisedExercise → UserEntry v2 → ... Each link is a traceable entity. |
