...
`/generate-reports`) now use `BasePage(STANDARD)`. Navigation via `/profile`
hub cards and `/transfer` dual-pane page.

## Files
- `layout.py` — `create_study_page()` wrapper using `SidebarPage`
- `sidebar.py` — `STUDY_SIDEBAR_ITEMS` (Submit, My Submissions, Exercise Reports, Activity Reports, Generate Reports)
