...
| `UserEntryOrchestrator` | `user_entry_routes.py` + 4 sub-factories | 9 → 1 | Successor to the former Submissions + Journal orchestrators (ADR-054 Commit 5c); eliminated multi-factory injection. `get_entry_report_view()` collapses the owner read (`EntryReportService.get_for_user`, ADR-088 §3) → revision lookup; `get_entry()` backs ownership-verified journal download |
