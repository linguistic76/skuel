...
| List sorts (`core/utils/entity_filters.py`) | sort by `sort_order()`; the view vocabulary IS the enum's three values (the day view orders by date, not priority — every priority renders there) |
