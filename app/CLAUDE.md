...
**MyPy:** `./dev quality` enforces **0 MyPy errors**. Per-module strictness overrides in `pyproject.toml`. Three globally-disabled codes: `type-var`, `var-annotated`, `type-arg`. `arg-type` is enforced on `core/` (2026-05-29) and `services_bootstrap/` (2026-05-30) via per-module overrides — enforces frozen-model / enum-NewType / typed-payload boundaries per the functional-direction roadmap, and at the composition root catches service↔protocol conformance gaps. Stays globally disabled for the still-un-swept `adapters`/`ui`/`tests`/`scripts` trees (a future sweep; the global-disable deletion is gated on those reaching 0). `assignment` is **enabled** — catches trailing-comma tuple bugs and real type mismatches. `core.services.*`, `core.ports.*` enforce `disallow_untyped_defs`. Domain backends suppress `misc` (MRO mixin conflicts). Narrow Neo4j property types with `int()`/`float()`/`str()` casts before arithmetic. Every new `Any` needs a `# boundary:` comment or should use a specific type.
