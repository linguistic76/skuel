...
**MyPy:** `./dev quality` enforces **0 MyPy errors**. Per-module strictness overrides in `pyproject.toml`. Three globally-disabled codes: `type-var`, `var-annotated`, `type-arg`. `arg-type` was re-enabled on `core/` (2026-05-29, via per-module override — enforces frozen-model / enum-NewType / typed-payload boundaries per the functional-direction roadmap; stays globally disabled for the un-swept `adapters`/`ui`/`services_bootstrap`/`tests`/`scripts` trees). `assignment` is **enabled** — catches trailing-comma tuple bugs and real type mismatches. `core.services.*`, `core.ports.*` enforce `disallow_untyped_defs`. Domain backends suppress `misc` (MRO mixin conflicts). Narrow Neo4j property types with `int()`/`float()`/`str()` casts before arithmetic. Every new `Any` needs a `# boundary:` comment or should use a specific type.
