...
**Evolution (2026-04-06):** Post-login redirect changed from `/profile` to `/home` — a new post-login landing hub with 6 navigational cards (Profile, Explore, Library, GradeBook, Workbench, Search) using `HubContainerGrid`. Hub view in `ui/home_hub.py`, route in `adapters/inbound/home_routes.py`. Navbar right section gains a Hub+Sign out dropdown (`_hub_dropdown()` in `navbar.py`); Sign out removed from the avatar dropdown (avatar now shows Profile + 6 Activity Domain links only).
