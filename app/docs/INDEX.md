...
| **[ADR-085: Ownership Read-Enforcement Contract](decisions/ADR-085-ownership-read-enforcement-contract.md)** | **Accepted (ownership bundle, 2026-08-21): every read on behalf of a user passes one of TWO chokepoints — `build_search_visibility_clause()` (search strategies + `get_visible_to_user` by-UID) or route-mediated `verify_ownership`; `get_visible_to_user` promoted to THE audience-aware service-to-service by-UID read; bare `get()` legal only as internal mechanics (post-verification / not-on-behalf-of-a-user / PUBLIC); NO third mechanism ever. Carries the G1–G7 read-side gap census as the closure worklist** |
