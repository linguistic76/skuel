...
| **Vault sync privacy wall** | One predicate (`is_ingestible_path`) at every ingestion door (`collect_files`, `ingest_file`, `reconcile_deletions`; reconciler + `/api/ingest/*` inherit): rejects **symlinks** (target may be external); applies a **`je_*` staging floor** scoped to the personal vault; enforces a **fail-closed allowlist** (`SyncAllowlist` / `SKUEL_VAULT_SYNC_ALLOWED_DIRS`, `periodic_notes/` default, dirs must be strictly under the root). Retroactive: narrowing the wall purges now-walled rows via reconciliation (full→smart auto-upgrade when governed). Content vault (outside the root) unaffected | Active (default-on) |
