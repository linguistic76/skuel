...
**C2 — Deletion is judged with a one-sync grace, never per file at ingest.** A cut-and-paste
is a deletion in the source note. A verdict taken while ingesting that note would cancel the
task and, when the destination ingests, mint a twin (Guard 4 ignores terminal twins) — the
exact shape the live W28→W29 fixture shows (below). So #1341's retirement becomes a
**tombstone** (`retired_at` on the edge, readers exclude it) and an **end-of-sync sweep**
deletes tombstones older than the sync's start. A 🆔 that reappears in any note within one
sync is a move and is re-pointed on the destination side; one gone for two consecutive syncs
is a deletion and the sweep applies rule 1. The order files ingest in cannot matter.
...
### Base line on the edge

`EXTRACTED_FROM` gains `source_line` (the raw line as last seen, 🆔 token included — the
parse base) and `retired_at` (tombstone). `create_extracted_from_links` sets `source_line` on
create and on the Guard 2b refresh (the refresh already rewrites the digest for a moved line;
the line text rides with it). `get_extracted_entities_for_entry` — the one read the guards and
the reconciler share — returns `source_line` and excludes tombstones. Existing edges have no
base: the first sight of a 🆔 line after PR 3 stores the base and applies **nothing**
(SKUEL's state stands, the outbound pass writes it back as today — the "Vacuum" case in the
fixture). One `./dev vault-sync --force` after PR 3 seeds every base at once.
...
this entry's edges own, and looks the rest up **user-wide, tombstones included** (new read:
`get_extracted_edge_by_vault_id(user_uid, X)` — `(task)-[r:EXTRACTED_FROM {vault_id: X}]->
(:UserEntry {user_uid})`). Found on another entry ⇒ a move: the edge is re-pointed (hard
delete of the old edge, keyed on `(task, X)` — #1341's `delete_extracted_from_links`; create on
this entry with the current digest and `source_line`) and the line takes the identity branch
above — reconciled against the base the old edge carried, so a check made *during* the move
still lands. Found nowhere ⇒ a phantom: today's behaviour, and the cleanup script's
`--repair-id` stays the repair for pre-🆔-era tasks.
...
### Deletion — tombstone, sweep, cancel

The #1341 pre-pass keeps its verdict (both keys gone) and its in-memory digest pruning, but
persists a tombstone (`retire_extracted_from_links`: `SET r.retired_at = datetime()`, keyed
on `(task, X)` as read) instead of the delete. `VaultReconciler.sync` gains an end-of-sync
step after ingest: `sweep_retired_extracted_from_links(user_uid, retired_before =
sync_started_at)` deletes every tombstone written in an *earlier* sync and returns
`(task_uid, vault_id, entry_uid)` rows; for each task that is still open
(`not status.is_terminal()`) the reconciler posts `update_task(status = CANCELLED)` through the
facade. Terminal tasks: nothing. A new counter, `VaultSyncStats.tasks_cancelled_by_deletion`,
renders only when nonzero ("N tasks cancelled — their lines were removed from the vault"),
beside "tasks re-opened". A cancel is a state change the user should see; a retirement alone
still is not (#1341's reasoning stands).

The mid-move window (cut, sync, paste, sync): the first sync tombstones, the second sync
re-points before its sweep runs — the tombstone is younger than that sync's start only if the
cut happened in it, so the grace is exactly one full sync. A paste that never comes is a
deletion.
...
to W29 with no twin minted, tombstone the two W28 originals whose 🆔s W29's twins already own
(they sweep as terminal, uncancelled, and become the script's RE-MINT proposals), and — after
PR 3 seeds bases — write Vacuum's `[x] ✅` into W29 on the following sync. That is the arc's
...
1. **Tombstone + sweep, no consequence yet.** `retired_at`; `retire_…` replaces the delete in
   the pre-pass; readers exclude tombstones; the reconciler's end-of-sync sweep hard-deletes
   tombstones older than the sync. Mutant: sweep deletes tombstones from *this* sync (a
   same-sync move would lose its edge).
2. **Moves re-point; retyped lines re-link.** The user-wide 🆔 read (tombstones included);
   re-point on the destination; Guard 4 writes the edge when the twin has none to this entry.
   Rig: cut a line from note A into note B in one edit → sync → one task, edge on B, no twin;
   then A-first and B-first ingest orders (name the files to force each). Mutant: lookup
   excludes tombstones (A-first order mints a twin). Post-merge: the W28→W29 fixture.
...
5. **Deletion cancels open tasks.** The sweep posts `CANCELLED` for open tasks; the counter
   and its fragment line. Rig: delete an open task's line, two syncs → cancelled; delete a
   done line → untouched; cut/paste across a sync boundary → moved, not cancelled. Mutant: the
   sweep cancels terminal tasks.
...
Sequencing note: 1 → 2 is fixed (moves need tombstones to be order-independent); 3 → 4 is
...
- **End-of-sync aggregation of per-file retirement lists instead of a tombstone**: keeps one
  edge shape but threads a new list through `ingest_user_entry` → `batch.py` →
  `IncrementalStats` → the reconciler, and still cannot see a paste one sync later. The
  tombstone is one property, one read filter, and a sweep.
...
- Two edge properties (`source_line`, `retired_at`); the hygiene audit and the cleanup
  script must read tombstones as absent.
