...
| **[ADR-066: Typed Update Intents (frozen `*UpdateIntent`, one update path)](decisions/ADR-066-typed-update-intents.md)** | **2026-06-04** | **Accepted (extends ADR-065): replace the unsound, decorative `*UpdatePayload` TypedDicts with frozen `*UpdateIntent` dataclasses (`UNSET` sentinel + `to_changes()`); collapse the four-way write boundary into ONE service-contract update path (validated + event-firing) plus an explicit `# raw-write:` bypass for full-DTO/system writes. Step 1 (Mapping bridge) shipped in PR #222; phased Tasks-first migration in [docs/roadmap/update-intents.md](roadmap/update-intents.md)** |
