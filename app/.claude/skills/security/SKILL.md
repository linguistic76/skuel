...
| **Vault sync privacy wall** | Fail-closed folder allowlist (`SyncAllowlist` / `SKUEL_VAULT_SYNC_ALLOWED_DIRS`): only opted-in folders under the personal vault root are ingested; everything else is walled off. Applied at the single ingestion chokepoint (`UnifiedIngestionService.ingest_directory`) so both the reconciler and `/api/ingest/*` inherit it | Active (opt-in) |
