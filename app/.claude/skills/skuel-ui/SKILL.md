...
- **GradeBook** — `render_gradebook_sidebar_page()` from `ui/gradebook/nav.py` — 4 items (My Submissions, Exercise Reports, Activity Reports, Revisions). Used on child pages: `/gradebook/mysubmissions`, `/exercise-reports`, `/activity-reports`, `/submit-activity-report`, `/activity-reports/detail`, `/revised-exercises`, `/revised-exercises/detail`. The hub at `/gradebook` is a container grid page (`BasePage(STANDARD)`, no sidebar) with 4 HTMX-loaded blocks — see `ui/gradebook/hub.py`.
