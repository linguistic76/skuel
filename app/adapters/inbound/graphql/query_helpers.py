...
class GraphQLQueryHelpers:
    ...
    async def get_prerequisites(context: GraphQLContext, ku_uid: str) -> list[KnowledgeNode]:
        ...
        Delegates to PsService.get_prerequisites() which returns
        Result[list[CurriculumDTO]].
        """
        if not context.services.ps:
            return []

        result = await context.services.ps.get_prerequisites(ku_uid)
    ...
    async def get_enables(context: GraphQLContext, ku_uid: str) -> list[KnowledgeNode]:
        ...
        Delegates to PsService.get_enables() which returns
        Result[list[CurriculumDTO]].
        """
        if not context.services.ps:
            return []

        result = await context.services.ps.get_enables(ku_uid)
        dtos = unwrap_result(result, [])
