...
`ACTIVITY_REPORT` is **not** a response to a specific artifact. It is a response to a user's aggregate activity over a period — a trailing window ending now, or a calendar month / ISO week (`core/utils/report_periods.py` is the vocabulary; the request form offers the current and previous month and ISO week). A calendar-period report generated before its period ends is *partial* and names what its counts cannot say (`metadata["limitations"]`: the task denominator is "open at period end" as far as the stamps can tell). The habit, goal and principle counters read persisted history — `HabitCompletion` rows, `Goal.progress_history` (appended by every progress writer), `Principle.alignment_history` (a self-assessment, a reflection, or a `seeded` entry) — never a latest-only stamp. **Deploy order:** `scripts/migrations/seed_activity_history_2026_09.cypher` runs BEFORE the history-reading code, giving every node that carries a stamp but no history one entry dated at its stamp; run after, the counters would read zero for those nodes until their next write.
