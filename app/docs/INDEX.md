...
> That date is the point of that table rather than derivable metadata, so it stays; every one of
> its rows carries a description as well.
>
> **The description cell is filled in progressively — fewer than half the data rows have one**
> (144 of 321 on 2026-09-24, table-header rows excluded; recount rather than trust it). An empty
...
| **[ADR-069: EXTRACT_ACTIVITIES Pipeline + EntryReport Convergence](decisions/ADR-069-extract-activities-pipeline-and-entry-report.md)** | **`Pipeline.EXTRACT_ACTIVITIES` wired — text → DSL parse → real entities with `EXTRACTED_FROM` provenance; ExerciseReport renamed EntryReport (`er_` UIDs); LLM journal responses as PRIVATE self-owned EntryReports** |
...
| **[ADR-087: Status-Guarded Conditional Writes](decisions/ADR-087-status-guarded-conditional-writes.md)** | **Accepted (conditional-write arc, 2026-08-24): every status write was a read-then-write whose read took no lock — measured 39/40 trials produced 2-4 concurrent writers each believing they performed the first completion, which is how a complete serialized after a reopen left `status=completed` with no `completion_date`. New `update_with_status_guard` on `_CrudMixin` (declared on `CrudOperations[T]`): the statement takes the node's write-lock BEFORE reading the prior (ADR-030's sentinel, single-statement form), applies `StatusWriteGuard`'s prior-conditional patches, and RETURNS the prior — services derive `is_completion_transition` / `is_reopen_transition` from it with the same pure helpers, so every `TaskCompleted` publish is transition-gated (a re-post writes nothing and announces nothing; the `is_repeat` flag this arc introduced was retired with the one-completion-door arc, D.0). Guarded-out is `Result.ok(applied=False)`, not-found is the error; auto-commit, no managed retry (at-most-once). `BaseService.update_status` deleted. Scoped OUT by name: counter races, non-status guards, edge CAS, Principles (prior-independent gate). today.js's request queue RETAINED at the time (the bundle itself was retired with the day-view arc D.1) — the primitive fixes each write's VERDICT, not the ORDER of two opposing requests** |
| **[ADR-088: Submit and Share](decisions/ADR-088-submit-and-share.md)** | **Accepted (Submit & Share arc, 2026-09-24; implementation pending — the arc's PR table is the ledger): two verbs — Submit asks for feedback and stays private (the GradeBook), Share lets groups or people see work, any time, no review gate (private and journal entries excepted). Two group-link kinds: `SUBMITTED_TO_GROUP` (a feedback request, read only by the group's owning teacher) vs `SHARED_WITH_GROUP` (every member) — two kinds, not a marker, because the group revoke deletes the edge whatever its properties. The share links are the only audience record and a link alone grants access; `visibility` keeps only private/public. DomainConfig gains `read_visibility` beside `search_visibility` (UserEntry opens to its audience, search stays owner-only — amends ADR-085 §2); Your wall is derived from the links; person shares need co-membership, the default group's roster excluded; `teacher:<group_uid>` keeps per-teacher targeting. Amends ADR-038, 040, 042 §7, 053 §1, 054 §3/§5/§6 and the 2026-09-21 sharing-door ruling, each in the PR whose code lands it** |
...
| [Askesis Intent Classification — Activation Arc](roadmap/askesis-intent-classification-activation.md) | The intent-conditioned Askesis layer, dormant since it was built, is ON: PR-2 (2026-08-31) kept the measured-best `mean` aggregation and moved the gate 0.65 → 0.35 — 19 of the 45 ratified queries fire, 0 wrong (`./dev eval-intent-classification`; PR-1 + baseline ratified 2026-08-31). The `AGGREGATION` carve-out was lifted the same day by the tool-selection first slice (a count question is answered by a vetted user-scoped tool or explicitly declined); the chunk-type filter stays hard-wired off as a gated PR-3 |
...
| **[Submit & Share Arc — Rulings & Contract](roadmap/submission-sharing-arc.md)** | **ACTIVE (ruled 2026-09-24): the 13-PR arc that splits submitting for feedback from sharing — founder rulings R1–R14, four refinements, the verified ground truth with the live census, the records ADR-088 amends and the PR that lands each note, per-PR scope with PR 0 verification findings, and the PR contract table whose Status column is the progress ledger (one PR per fresh context)** |
...
|----------|-----------|-------------|
| [Vault Re-Sync Never Retracts a Share](roadmap/done/vault-resync-never-retracts-a-share.md) | 2026-09-26 | The write-once-widen vault `audience:` gap, closed by the Submit & Share arc PR 8 (R9): a vault note is a draft, never shared, and `status: submitted` files a frozen copy once per authored snapshot — a re-sync has nothing to retract |
| [Docs De-Fiction Pass — Build Brief](roadmap/done/docs-defiction-pass.md) | 2026-09-22 | Seven PRs (#1379–#1390, closed by #1394): the route-claim scanner built on the runtime route table (a strict superset of the AST catalog it replaced — 891 vs 475 paths), a `file.py:N` range check inside the link checker, and `history-in-code --docs`; then the sweep — `docs/features/` and the SEL doc deleted, `OWNERSHIP_VERIFICATION.md`'s 47-route changelog dropped, domains/skills/patterns repointed to registered routes, the inbound docstrings swept, and `decisions/` resolved standing-contract-by-section against reproduced successors. The `fiction` class **stays advisory by ruling**: both precision draws came in under the 95% bar (15/30 and 17/30), and the two biggest false-positive classes are grammar the scanner deliberately does not carry. Holds the classes, the D2/D3 policy, the five rulings and the residuals the (b) and (d) queues inherited |
| [Tasks+ on Phones, the /profile Hub, and One Chrome for the App](roadmap/done/tasks-plus-one-chrome.md) | 2026-09-19 | Six PRs (#1373–#1378) from a 34-agent review: the below-`lg` sidebar row is a nav list with `aria-current` and a parse-time centring script; phone geometry (navbar `h-14` border included, safe-area bottom nav); badges opt-in on `intersect once` (the per-page MEGA-QUERY on phones gone); ONE navbar for every role — one door per section from `ICON_NAV_ITEMS`, Tasks+ the door, role doors at lg+, no admin fork/hamburger/Calendar icon/Today tab, `/` → `/today`; `/profile` retired to a 404 (the `AssessmentService` stack went with it). Holds the measurements, the six rulings overturned, every PR's amendments and the chrome gate (`scripts/chrome_gate.py`) |
| [R4 Vault Inbound Propagation — Build Plan](roadmap/done/r4-vault-inbound-propagation.md) | 2026-09-16 | The vault became the place tasks are edited (four PRs, #1343–#1345 + docs): a vault-side check, uncheck or field edit of a 🆔 line reaches its task by a three-way merge against the line as SKUEL last saw it (`EXTRACTED_FROM.source_line`), one `update_task` per line, the base advancing only on ok and with SKUEL's own write-backs; a moved line is re-pointed in one statement; a line gone for two syncs cancels an open task and leaves a done one (the grace stamp on Task, the end-of-sync sweep, its hold over any incomplete inbound pass). Holds the three constraints (C1–C3), the rulings, the W28→W29 live fixture and the residuals. ADR-070 Decisions 1–3 state the mechanism. |
| [Line Deletions Leave `EXTRACTED_FROM` Edges](roadmap/done/line-deletions-leave-extracted-from-edges.md) | 2026-09-15 | A 🆔 task line deleted from a note that survives was the one deletion propagation could not see — its edge and digest fed the extraction guards forever and swallowed the same text typed back. The pre-pass now retires an edge whose line is gone by BOTH keys (🆔 absent, digest on no 🆔-less line); the task stays. A stripped token is recognised by hash and re-minted by the reconciler; the 🆔 is written in front of a trailing ✅ so the next sync no longer appends a second date |
...
| [Periodic-Note Doors — one "Notes" picker](roadmap/done/period-notes-toolbar-picker.md) | 2026-09-05 | The top-level door to the quarterly and yearly notes. Mike's ask was "a simple icon"; the real problem was crowding (four note pills in a toolbar that already wraps on a phone) and the fact that no mark in the 159-icon set says QUARTER or YEAR. Chosen from four rendered options: the note family collapses into one "Notes" disclosure listing all five periods on Week/Month/Today. The row for the surface's own period follows the VIEW; the rest open the CURRENT period, made legible by each row's trailing short label. **Retired 2026-09-18** — the picker is deleted; the sidebar's Journal row and the in-note period rail are the doors |
| [Periodic-Note Sidebar — week rail + period rail](roadmap/done/periodic-note-sidebar-rail.md) | 2026-09-05 | The in-note door, remade after Mike ruled the "up"-links ladder unusable. Two pickers with no overlap: the mini month grows the ISO-week rail the full month view already has (weeks open weekly notes, days open daily ones), and below it a period rail carries all five kinds on every note — icon, the period named in full, and ‹ › arrows to its neighbours. The wider-periods-only ladder and the single-period bottom bar both go; `period_step` joins `period_link` as the one neighbour derivation, so a December→January or Q4→Q1 arrow carries the year |
| [Monthly-Note Panel Parity](roadmap/done/monthly-note-panel-parity.md) | 2026-09-05 | The monthly note's read-only planning panel, built on Mike's ruling after the lived-use gate was re-derived and found unfired (vault Monthly/ empty, both graph monthly notes empty). `week_panel.py` → `period_panel.py`; `planning_period` is the one place a note kind resolves to its range — a quarterly/yearly note adds one branch there. *Panel removed 2026-09-18: the note page now carries the Tasks+ sidebar and the period navigator instead* |
| [Docs `updated:` Frontmatter — Auto-Stamp](roadmap/done/docs-updated-auto-stamp.md) | 2026-09-01 | Stamper + backfill + guard shipped (#1212); kept for the settled forks (7-day rot threshold, scope, generated-doc exclusion) and the permanent rules — never cite `updated:` as staleness evidence outside the guard; no contradictory-prose detector |
| [Backend-Typing Follow-on](roadmap/done/backend-typing-follow-on.md) | 2026-08-21 | The three items that outlived the backend-typing arc, each a decision rather than a retype: the severed `DomainConfig` prerequisite chain reconnected as enums (#1106), the `PsOperations` dual-layer doctrine measured and found to be fiction (#1107), and the lying `ku_backend` fixture (#1113). Ruled *state the layer, don't rename* for the `Ku`/`Ps`/`Lp` `*Operations` trio |
| [Substance-Write Grain — the `ku_uid` That May Not Be a Ku](roadmap/done/substance-write-grain.md) | 2026-08-21 | Closure record + investigation record in one document (#1113): substance-counter grain is **agnostic by ruling** — `knowledge_uid` may name a Ku or a PathStep and the write lands either way. The `WHERE ps IS NOT NULL` row-gating that reported `ok(0)` for landed writes is fixed and never to be resurrected |
| [The Reopen Vault Surface](roadmap/done/reopen-vault-surface.md) | 2026-08-24 | Both PRs (#1151, #1152): `WriteResult.updates_applied` carries per-update outcomes so a minted 🆔 persists only when its own injection landed (fail-closed), and a task reopen un-checks its Obsidian line and strips the `✅` date — driven by a state predicate, never by `TaskReopened` |
