...
| Dead-doc-links scanner narrowings — **2 measured items, scheduled 2026-09-02** (the placeholder guard is consulted by only one of four passes; a vocabulary gap for documentation metavariables) | Mike schedules it — approved work, not waiting on data. Cheap and self-contained; a natural ride-along on the next touch of `scripts/health/dead_doc_links.py` | Spec in [`dead-doc-links-sweep-queue.md`](dead-doc-links-sweep-queue.md) § Scheduled: two scanner narrowings — each with its measured shape, effect (−5 and −8) and its collision check against the tracked tree. ⚠️ Item 1 is a precondition for FOUR of item 2's eight (the bare-pass ones); one target is deliberately left reported rather than earning a one-off rule; and the obvious wider rule (reject uppercase stems) was measured and REJECTED — it swallows real doc names. Re-derive the counts before building |
