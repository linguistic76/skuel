...
The cross-user negative controls are the point: private vault notes live
...
async def _seed_entries(user_entry_service, seed_user) -> dict[str, str]:
    """Create two owner entries (knowledge + reference pipeline) and one
    ...
    specs = [
        ...
        (
            ...
            OWNER_UID,
            "Morning reflection",
            "Private reference text mentioning zettelkasten workflow.",
            Pipeline.REFERENCE,
        ),
        (
            "other_users",
...
async def test_owner_search_finds_entry_by_title_and_content(
    ...
    assert {e.uid for e in by_content.value} == {created["reference_note"]}
...
async def test_cross_user_negative_control(
    ...
    # OTHER searching for OWNER's private reference text: nothing
    reference_probe = await search_router.search(
        EntityType.USER_ENTRY, "zettelkasten", user_uid=OTHER_UID
    )
    assert reference_probe.is_ok
    assert reference_probe.value == []
    ...
    assert created["reference_note"] not in uids
...
async def test_faceted_search_owner_scope_and_pipeline_facet(
    ...
    # Pipeline facet (category_field="pipeline"): reference-only slice
    reference_request = SearchRequest(
        query_text=None,
        entity_types=[EntityType.USER_ENTRY],
        extended_facets={"pipeline": Pipeline.REFERENCE.value},
    )
    reference_faceted = await search_router.faceted_search(reference_request, user_uid=OWNER_UID)
    assert reference_faceted.is_ok, reference_faceted.expect_error()
    reference_uids = {r.get("uid") for r in reference_faceted.value.results}
    assert reference_uids == {created["reference_note"]}
