...
Listens for ReportSubmitted events with a fulfills_project_uid.
...
async def handle_assignment_submission(
    ...
    """
    Handle ReportSubmitted events that fulfill an assignment.
    ...
    Args:
        event: The ReportSubmitted event
    ...
    logger.info(
        f"Assignment submission detected: ku={event.report_uid} -> project={event.fulfills_project_uid}"
    )

    result = await reports_core_service.process_assignment_submission(  # type: ignore[attr-defined]
        ku_uid=event.report_uid,
        project_uid=event.fulfills_project_uid,
    )

    if result.is_error:
        logger.error(
            f"Failed to process assignment submission: ku={event.report_uid}, "
            f"project={event.fulfills_project_uid}, error={result.error}"
        )
    elif result.value:
        logger.info(
            f"Assignment submission processed: ku={event.report_uid} -> project={event.fulfills_project_uid}"
        )
