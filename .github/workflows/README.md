...
| **Dependency CVE Audit** | Job in `ci.yml` | This repo | ✅ status check | When `app/**/*.py`, `pyproject.toml`, or `uv.lock` change — `pip-audit` over the full locked resolution (`scripts/audit_dependencies.sh`, same as `./dev audit-deps`); accepted findings documented in `app/.pip-audit-ignore` |
