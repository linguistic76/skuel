...
- **Submissions** — `render_submissions_sidebar_page()` from `ui/workbench/nav.py` — 4 items: Exercise (`/submissions/exercise`), Journal (`/submissions/journal`), Sync (`/submissions/sync`), History (`/submissions/history`). Root `/submissions` is a sidebar-free MOC page with 4 cards. `/submit` → 302 → `/submissions/exercise`; `/settings/vault` → 301 → `/submissions/sync`.
