...
**Two doors (discussion-first, `docs/roadmap/done/journals-discussion-first.md`):** typed text (`POST /journals/start`) opens an open, user-led **discussion** for both tiers (`run_discussion`, companion voice, no analysis template); files/audio (`POST /journals/upload`, `POST /journals/folder-process`) run the **DNWF** processing path. The Scribe → Thought-Partner → What-Is-Related staging lives on the file/audio door only, not on typed text.

**Persistence (ADR-073 + ADR-078, opt-in):** Journals are private on **two independent axes** — *understanding* (does it feed SKUEL's model of you? **never**, for any chat, saved or not — the vault doorway is the only channel in) and *persistence* (is it stored at all? **not by default** — a chat is ephemeral until the user explicitly saves it). The **file/audio door** is fully **zero-persistence** — it writes the processed output to the user's own `je_out/` folder (see §7), writing nothing to Neo4j. Every discussion (both doors) is **ephemeral by default** (ADR-078 §5, founder realignment 2026-07-13): its memory lives in a client-side transcript accumulator that dies on reload, and it reaches the **owner-private `:ConversationSession` + turns** store **only when the user explicitly presses *Save this chat*** (`POST /journals/save`) — there is no auto-save. The store is **understanding-agnostic**: even a *saved* chat writes **nothing** to the understanding channel (no `UserEntry`, no embeddings, no search/context), so ADR-073's understanding wall (§2) holds intact. Saved discussions are revisitable/continuable via `/journals/discussion/{id}`; the user still keeps whatever they copy or download. **Both doors share one ephemeral substrate** (a structured `transcript_json` accumulator): the typed door opens on the entry→reply pair, the file/audio + DNWF doors open on the source→output pair (the file text/transcript as the user turn, the compiled `_out.md`/Stage-3 output as the assistant turn) — both then expose the identical *Save this chat*. (There is no separate flat accumulator; it was removed with P3 PR2.) Audio transcription is synchronous and file-based (decision A). Periodic notes remain a deliberate stored feature.
...
**UserContext digest:** `JournalService._build_context_summary()` grounds every turn on the canonical `UnifiedUserContext.build()` (~150-field standard depth, never `build_rich()`/ZPD — ADR-081 D2) rendered through the **named curated projection** `render_journal_grounding` (`core/services/journal/grounding_projection.py`). The projection's `JOURNAL_GROUNDING_FIELDS` is the explicit UserContext field list (identity; active goals/tasks/habits with light relevance — progress %, overdue/due-today, streaks; current path steps + mastery counts), unit-enforced by a recording-context test so the prompt stays a legible rendering, never an open dump. UserContext supplies the relevance lens; the optional Goals/Tasks/Habits services supply titles (standard build is UID-depth). An unwired `context_builder` or a failed build degrades to the plain six-titles digest (the pre-ADR-081 floor). The digest ends with a "Personal project notes" section of up to 8 vault-synced notes (title + 300-char snippet, newest first) — `pipeline=knowledge` UserEntry nodes (developed files in the `knowledge/` doorway, shared to teach SKUEL; a consented `je_pro/` file carries the same pipeline) whose `metadata["vault_file_path"]` marks them as synced from `VAULT_ROOT` rather than created via the journals UI. `UserEntryService` is required and always present.
...
| Instruction source | Committed floor dicts in `instruction_loader.py` (`_DISCUSSION_BASE_DEFAULTS` / `_FOLLOW_UP_BASE_DEFAULTS`, per mode) + optional founder-local `data/instructions/journals.discussion.{mode}.md` / `journals.follow_up.{mode}.md` override (ADR-081 D1) | Files from `data/instructions/` |
...
The STANDARD tier's discussion/follow-up bases are a committed default floor — guaranteed present per mode, so the companion is fully operational with no file setup — with an optional founder-local override file replacing the floor when present and non-blank (ADR-081 D1: authored + refinable, never broken; a blank override degrades to the floor). The FOUNDER stage tier is file-driven because its orientation is proprietary and lives outside the codebase.
...
| `instruction_loader` | `core/services/journal/instruction_loader.py` | Prompt composition. Discussion/follow-up bases = committed floor + optional `data/instructions/` override (ADR-081 D1); FOUNDER DNWF stages load from `data/instructions/`. |
| `grounding_projection` | `core/services/journal/grounding_projection.py` | The named curated rendering of `UnifiedUserContext.build()` (ADR-081 D2). `JOURNAL_GROUNDING_FIELDS` = the explicit field list (scope-creep mitigation, test-enforced); `render_journal_grounding()` is a pure function joining the context's relevance lens to fetched titles. |
...
| Routes | `adapters/inbound/journals_routes.py` | FOUNDER enforcement (`journal_tier.is_founder()`) lives at the route layer. Text-entry path: `POST /journals/start` runs AI and returns the response **inline** (`HX-Retarget` `#journal-workspace`) — **ephemeral by default** (ADR-078 §5): it persists **nothing** (the opening pair rides the composer as a structured `transcript_json` accumulator), **no `UserEntry`**, no redirect. `POST /journals/follow-up` selects one of two memory models: session-backed (a *saved* chat — reads/appends turns from Neo4j, 404-not-403 on a non-owner) or ephemeral-structured (every unsaved discussion — both doors — grows the client-side `transcript_json` via OOB swap). `POST /journals/save` is the single explicit persistence gesture — folds the ephemeral transcript into an owner-private `:ConversationSession` + turn pairs in ONE transaction (`ConversationService.save_transcript` → `ConversationBackend.save_transcript`, atomic so a concurrent read never sees a partial session), swaps the composer to its session-backed shape, and OOB-refreshes the revisit list. The file/audio + DNWF doors share this same substrate — their composer opens on the source→output pair (`FileOutputFragment` / `Stage3Fragment` carry a `transcript_json`). File-upload/`folder-process` paths process to the user's own `je_out/` folder via one shared stateless batch engine — `JournalBatchService` in `core/services/journal/journal_batch_service.py` (also no `UserEntry`; routes only parse, gate, and render its `BatchRunReport`); FOUNDER audio → transcript review → Scribe. Response fragments carry a lazy-loaded "Suggested activities" panel (`POST /journals/suggest-activities`, FULL tier) that takes the reflection **content in the body** — no stored entry to read or cache. `GET /journals/{entry_uid}` is **periodic-notes-only** (the `PERIODIC_NOTE_KINDS` ladder: daily/weekly/monthly/quarterly/yearly); any non-periodic uid → 404; every kind **but daily** additionally carries a read-only right panel of its period's Tasks+Events + Milestones (`ui/journals/period_panel.py` — `planning_period` resolves the ISO week / calendar month / quarter / year, via `CalendarService.get_planning_items` — periodic-notes arc S3; rows door to `/today/{date}`, daily stays panel-less, and the two long periods sub-head their rows by month). The sidebar's **period rail** carries one row per period kind (daily → yearly) on every periodic note — each row opens that period's note and steps to its neighbours — above it a mini month grid whose day cells and ISO-week rail open the daily and weekly notes directly. It is the in-note door to every period, and the one prev/next mechanism (it replaced the wider-periods-only "up" ladder, #1277). The top-level door is the navbar **"Notes" picker** (`ui/layouts/period_notes.py`), which lists all five periods on every page and opens the period the page is showing (derived from the request path); both derive their URLs from `ui/journals/period_links.py`. `GET /journals/je-out/{filename}` serves flat `je_out/` files (`.md`/`.txt`) with a path-containment guard. The **discussion revisit surface** (ADR-078) lives at `/journals/discussion/{session_id}` — GET continues (rehydrate workspace from stored turns + restored source selection), `…/delete` (POST) drops the session subtree, `…/export` (GET) streams a markdown transcript, `…/rename` (POST) re-titles it; the `/journals` sidebar lists the user's owned sessions. Every op is owner-scoped (404-not-403). |
...
`JournalService` takes `LLMCallerProtocol` + `UserEntryService` (required) and `GoalsService` / `TasksService` / `HabitsService` / `UserContextBuilder` (all optional — omitting the context builder degrades grounding to the plain title digest; omitting the domain services too degrades `_build_context_summary()` toward an empty string).
...
**Exemplar-guided processing (`je_raw`/`je_pro`, ADR-073 §4).** Matched `je_raw`↔`je_pro` pairs (by filename stem) are read *off disk at processing time* by `JournalBatchService._load_exemplars()` and injected into the STANDARD journal prompt (the service's LLM compile path) as bounded few-shot examples (≤3 pairs, each truncated) — teaching the pipeline *how the user likes journals processed* (style), never facts about the user. Exemplar use is read-only and in-memory; absent/unmatched folders degrade cleanly to the no-exemplar prompt. The loader skips `je_use: understanding` files and strips YAML frontmatter from exemplar text (consent metadata is not style). `Pipeline.REFERENCE` is **reserved** for a future per-user *stored* exemplar layer and has no producer today.
...
- `Pipeline.allows_sharing()` returns `False` for `REFERENCE` and `TRANSCRIBE_AND_STRUCTURE`. No audience picker is offered in the journals UI, and a saved chat is owner-private with no sharing surface at all (ADR-078). ⚠️ The pipeline guarantee rides on the **pipeline**, so it does not extend to a note stored under a shareable one — `extract_activities` and `knowledge` permit sharing. The vault/YAML door defaults an absent `audience:` to `teachers` for submission-shaped pipelines only (`Pipeline.shares_by_default()`); a vault note — `knowledge` or `extract_activities` — with no `audience:` is **private** and shares only by explicit audience (rulings 2026-09-02). ⚠️ A re-sync never retracts an existing share — notes synced under the old default keep theirs until retracted (`scripts/retract_defaulted_vault_note_shares.py`, one-shot).
...
- **Richer UserContext injection** — ✅ shipped (ADR-081 D2): grounding now rides `UnifiedUserContext.build()` through the curated projection (relevance-annotated goals/tasks/habits, learning journey). Further enrichment = extending `JOURNAL_GROUNDING_FIELDS` deliberately, not new plumbing.
