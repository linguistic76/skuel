...
**Key Files:** `/ui/layouts/base_page.py`, `/ui/layouts/navbar.py`, `/ui/patterns/sidebar.py`, `/ui/patterns/` (PageHeader, form_generator, card_generator, etc.), `/ui/explore/nav.py` (Explore graph-centered sidebar), `/ui/explore/graph.py` (ExploreGraphView — Vis.js sidebar graph component), `/ui/learning_loop/` (exercise status pills, PS submissions/feedback renderers — shared with Library), `/ui/teaching/nav.py` (Teaching sidebar), `/ui/teaching/student_hub.py` (Student hub), `/ui/page_contexts.py`, `/ui/tokens.py` (spacing/layout), `/ui/palette.py` (centralized hex colors for visualization), `/ui/visualization/` (Chart.js/Vis.js/Gantt presentation adapters)
