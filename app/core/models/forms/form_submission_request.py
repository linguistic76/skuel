...
class FormSubmissionCreateRequest(BaseModel):
    ...
    form_template_uid: str = Field(
        ..., description="UID of the FormTemplate being responded to"
    )
    ...
    group_uid: str | None = Field(
        None, description="Share with this group on submit"
    )
    recipient_uids: list[str] | None = Field(
        None, description="Share with these users on submit"
    )
    share_with_admin: bool = Field(
        False, description="Send to admin on submit"
    )
