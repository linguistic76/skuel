...
| [Dead-doc-links sweep queue](roadmap/dead-doc-links-sweep-queue.md) | The live-docs residue of the B1–B4 arc (871 → 343): per-directory split, the tail shape that decides the fix (most targets have no same-basename match, so the usual fix is the citing prose — but a rename looks the same, so investigate first), heavy hitters, ride-along protocol — plus disproven claims about files that exist, which the scanner is blind to |
