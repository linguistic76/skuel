...
- `Pipeline.allows_sharing()` returns `False` for `REFERENCE` and `TRANSCRIBE_AND_STRUCTURE`. No audience picker is offered in the journals UI, and a saved chat is owner-private with no sharing surface at all (ADR-078). ⚠️ The pipeline guarantee rides on the **pipeline**, so it does not extend to a note stored under a shareable one — `extract_activities` and `knowledge` permit sharing by explicit audience.
