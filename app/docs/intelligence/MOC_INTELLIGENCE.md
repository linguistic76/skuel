...
**Last Updated:** January 20, 2026 · **Code-accuracy audit:** August 8, 2026 (replaced a stale `MocNavigationService`/`MOCService` class-stack description — those services were deleted with the MOC Ku type; see below)
...
| ORGANIZES operations (create/read/reorder) | Backend `_OrganizesMixin` (`adapters/persistence/neo4j/_organizes_mixin.py`) is **entity-generic** — its Cypher matches any `:Entity`, consistent with emergent (any-Entity) MOC identity. Surfaced via `PsOrganizationService` + the `PsService` facade, which narrow it for the unauthenticated PathStep API (a PathStep subject through `ps_core.get()`; organizers, children and roots scoped to `SHARED_CURRICULUM_TYPES` on the query — a personal `moc: true` map is never read through that door); **8** have routes in `adapters/inbound/path_steps_api.py` (`organize` / `unorganize` / `reorder` / `get_organized_children` / `is_organizer` / `get_organization_view` / `find_organizers` / `list_root_organizers`), and `get_navigation` is service-only (no route). Every read and `organize` validate a PathStep subject (`ps_core.get()`); only `unorganize` and `reorder` act on an existing edge by its two uids without one (admin-only at the route). |
...
> **Stale description (corrected 2026-08-08 audit).** Earlier revisions described a `MOCService` →
> `MocNavigationService` → `KuService` **class stack** (files `moc_service.py`,
> `moc_navigation_service.py`) with **KU-scoped** methods `is_moc(ku_uid)` /
> `get_moc_view(ku_uid)` / `find_mocs_containing(ku_uid)` / `list_root_mocs()`. **That class
> architecture is gone** — `MOCService` was deleted when the MOC Ku type was removed (ORGANIZES generalised to
> all Ku types), and `MocNavigationService` is the ancestor, via `KuOrganizationService` and the Article →
> Lesson → PathStep renames, of today's `PsOrganizationService`; none of the KU-scoped names is live.
>
...
> (the real, PathStep-scoped equivalents of the former KU-scoped names above).
