...
SKUEL is built on a developing ontology — 21 entity types, behavioral traits, relationship patterns, and cross-cutting systems that together describe a learner's world. The type system is how that ontology becomes executable:

| Ontological Concept | Type Expression |
|---------------------|-----------------|
| "There are 21 kinds of things" | `EntityType` enum with 21 values |
...
| "Query results have known shapes" | 93 TypedDicts in `core/ports/query_types.py` |
...
| Typed protocol returns | ~149 methods return specific models/TypedDicts, 0 `Result[Any]` in protocols (1 intentional in `base_service_interface.py`) |
| Query type coverage | 93 TypedDicts (21 input, 72 output) |
...
- [Entity Type Architecture](ENTITY_TYPE_ARCHITECTURE.md) — the 21 entity types and behavioral traits
