...
- **Admin navbar:** SKUEL logo (left, → `/`) + empty center + avatar (→ `/`) + Sign out. Admin home hub at `/` shows two cards: Admin (`/admin`) + Teaching (`/teaching/students`). Mobile: hamburger with Admin + Teaching links. Icon links hidden for admins.
