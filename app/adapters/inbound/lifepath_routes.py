"""
LifePath Routes for SKUEL
=========================

Domain #14: The Destination - "Everything flows toward the life path"

Routes for:
- Vision capture: User expresses vision in their own words
- LP recommendations: Suggest Learning Paths based on vision themes
- Designation: Set a Learning Path as the life path
- Alignment: View word-action alignment dashboard
- Recommendations: Get personalized life path recommendations
...
from core.ui.daisy_components import Button, Card, Div, Label, Progress, Span, Textarea
from core.utils.logging import get_logger
from ui.tokens import Container

logger = get_logger("skuel.routes.lifepath")
...
def create_lifepath_routes(_app, rt, services):
    """
    Create LifePath routes.

    Routes:
    - GET /lifepath - Main life path dashboard
    - GET /lifepath/vision - Vision capture page
    - POST /lifepath/vision - Process vision capture
    - GET /lifepath/recommendations - LP recommendations
    - POST /lifepath/designate - Designate an LP as life path
    - GET /lifepath/alignment - Alignment dashboard
    - GET /api/lifepath/status - Get full status (JSON)
    - POST /api/lifepath/vision - Capture vision (JSON)
    - POST /api/lifepath/designate - Designate life path (JSON)
    - GET /api/lifepath/alignment - Get alignment data (JSON)
    """
    ...
    async def lifepath_dashboard(request: Request) -> Any:
        ...
        # Get full status
        lifepath_service = services.lifepath
    ...
    logger.info("LifePath routes registered")
...
def _build_recommendations_page(data: dict, user_uid: str) -> Any:
    ...
    return Div(
        ...
        P(f'Your vision: "{vision.get("statement", "")}"', cls="text-base-content/70 italic mb-2"),
        P(
            f"Themes extracted: {', '.join(vision.get('themes', []))}",
            cls="text-sm text-base-content/50 mb-8",
        ),
