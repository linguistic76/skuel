...
| `adapters/inbound/journals_routes.py` | FOUNDER tier enforcement lives here; the typed discussion door (`/journals/start` + session-backed `/journals/follow-up`) persists an owner-private `:ConversationSession` + turns for revisit/continue (ADR-078) but **no UserEntry** (understanding-agnostic — ADR-073's wall holds); the file-upload path is fully zero-persistence (ADR-073), processing to the user's own flat `je_out/` folder via one shared batch engine; `GET /journals/{entry_uid}` is **periodic-notes-only**; `POST /journals/suggest-activities` takes reflection content in the body and returns the lazy-loaded suggestions panel; `GET /journals/je-out/{filename}` serves flat `je_out/` outputs |
