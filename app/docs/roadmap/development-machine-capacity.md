...
check: "`free -g` (total ≥ 31) and `nvidia-smi --query-gpu=memory.total --format=csv` (≥ 16384 MiB); then re-measure every row below on the new box before touching its bound"
---
...
| serial composed session | `comprehensive` / `integration` / `quick` gaining `-n N --dist loadfile` — N sets of three containers | peak container memory of one set (below) × N, plus one app boot per worker; whether the integration modules are xdist-safe (a fixed port, a shared vault path, module state) — that is a **test-design** question the memory question hides behind | `loadfile` distribution (module-scoped fixtures); a worker still builds its own set — sharing one container across workers is a different design, not a knob |
