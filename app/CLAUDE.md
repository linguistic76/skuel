...
**Dead-code detection (advisory):** `./dev bloat` — staged-but-unwired work belongs in `PLANNED_EVENTS`/`PLANNED_METHODS`/`PLANNED_TEMPLATES` in `scripts/detect_bloat.py`, not the trash — but a **stale** registration is a WARNING and fails `--check`. Stale means PROVABLY stale: the subject vanished (any tier), or an event is now genuinely published. Where wiring cannot be attributed — a name-masked method, a receiver-blind template match — the entry is kept and flagged, never demanded. Scope = events/methods/templates ONLY — a clean run is NOT evidence for fields, dataclasses, enum members, or config knobs (those are found by review). **See:** `/docs/tools/BLOAT_DETECTION.md`
