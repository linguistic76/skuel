...
- `GET  /journals/{entry_uid}` — **periodic-notes-only** (`entry_kind` ∈ `PERIODIC_NOTE_KINDS`: daily, weekly, monthly, quarterly, yearly) → `PeriodicNotePage` with a compact calendar navigation sidebar (mini month grid, ← Calendar link, prev/next period nav, and a period ladder of "up" links to the wider periods the note sits inside — the in-note door to them; the top-level door is the navbar's "Notes" picker). Every kind but daily also carries the read-only planning panel of its period's Tasks+Events + Milestones; quarterly and yearly sub-head those rows by month. Any non-periodic uid → 404 (discussions are served by `/journals/discussion/{session_id}`, not this route).
