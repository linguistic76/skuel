...
class GoalsCoreService(
    ...
    async def update_goal(self, uid: str, intent: GoalUpdateIntent) -> Result[Goal]:
        ...
        # goals_progressed counter reads. On a completion the figure and this stamp
        # move onto the transition condition below (``_with_completion_progress``).
        ...
        # sets a completion stamp while naming no status claims the goal is completed,
        ...
        if not outcome.applied:
            return Result.fail(stranded_stamp_error(EntityType.GOAL, outcome.prior_status))
