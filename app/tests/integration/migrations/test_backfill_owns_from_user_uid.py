...
5. The property==:OWNS-owner invariant (restricted to existing, non-system
...
async def test_backfill_creates_owns_edges_from_user_uid(
    ...
    # 5. Global invariant: every user_uid-bearing Entity whose (non-system)
    # owner exists has its :OWNS edge
    assert (
        await _count(
            neo4j_driver,
            "MATCH (e:Entity) "
    ...
    # 6. Idempotence — re-running creates nothing new
    await _run_migration(neo4j_driver)
