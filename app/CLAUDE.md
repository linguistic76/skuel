...
- **Request Bodies (POST):** Pydantic request models — `parse_body(request, Model)` at a door both API clients and HTMX forms reach (JSON or a form encoding by `Content-Type`, one validator; the CRUD factory's create/update, the admin account actions), `parse_json_body` / `parse_form_body` where only one caller kind exists, or `body: Model` in the signature (FastHTML binds it during parameter extraction, *before* the handler and `@boundary_handler` run, so `install_request_validation_guard` is what converts a rejected model; ⚠ never annotate an auto-bound body field as a `Literal` — FastHTML calls the annotation to coerce and `Literal(...)` raises `TypeError`, which no guard converts). htmx never JSON-encodes a body — an `hx_headers` Content-Type override does not change what it sends.
