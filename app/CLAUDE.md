...
- **Regular user navbar** icon links (left section, in order): **Hub** (`/home`) + **Tasks+** (`/profile`) + **Explore** (`/explore`) + **Library** (`/library`) + **Submissions** (`/submissions`) + **GradeBook** (`/gradebook`) + avatar **U** (click → Profile + 6 Activity Domain links + Sign out dropdown). Right section: **Search** (`/search`) icon + notification bell. After login, regular users land on `/home`.
...
- `/home` is the **post-login landing hub** — 3x2 `HubContainerGrid` with 6 navigational cards (Tasks+, Explore, Library, Submissions, GradeBook, Settings); hub view in `ui/home_hub.py`, route in `adapters/inbound/home_routes.py`.
