...
**Activity Domains are equal entry points.** A user's Tasks, Goals, Habits, Events, Choices, and Principles are knowledge-in-action. They receive the same feedback infrastructure as curriculum work — via `ActivityReport` entities generated by AI or written by admins.
...
| `SUBMISSION_FEEDBACK` | Teacher **or AI** | Assessment with `subject_uid` pointing to student's submission | `HUMAN` (teacher) or `LLM` (AI) |
...
Both teacher feedback and AI feedback produce `SUBMISSION_FEEDBACK` entities — same entity type, different `processor_type`:

| Feedback Source | Service | ProcessorType | Entity Created |
|----------------|---------|---------------|----------------|
| Teacher | `TeacherReviewService.submit_feedback()` | `HUMAN` | `SUBMISSION_FEEDBACK` |
| AI (via Exercise) | `FeedbackService.generate_feedback()` | `LLM` | `SUBMISSION_FEEDBACK` |
